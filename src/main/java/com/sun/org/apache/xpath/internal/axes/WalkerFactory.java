...
if (DEBUG_ITERATOR_CREATION)
  ...
  static boolean isProximateInnerExpr(Compiler compiler, int opPos)
  {
    ...
    switch(op)
    {
      ...
      case OpCodes.OP_LOCATIONPATH:
        break; case OpCodes.OP_FUNCTION:
          ...
          throws javax.xml.transform.TransformerException
  {
    ...
    while (OpCodes.OP_PREDICATE == compiler.getOp(predPos))
    {
      ...
      {
        case OpCodes.OP_VARIABLE:
                return true; case OpCodes.OP_LOCATIONPATH:
          break;
        case OpCodes.OP_NUMBER:
        case OpCodes.OP_NUMBERLIT:
          return true; case OpCodes.OP_FUNCTION:
          boolean isProx
            ...
            throws javax.xml.transform.TransformerException
  {
    ...
    int stepType;
    int stepCount = 0;
    int analysisResult = 0x00000000;  while (OpCodes.ENDOP != (stepType = compiler.getOp(stepOpCodePos)))
    {
      stepCount++;
...
|| (Axis.PRECEDING == axis)
         ...
         whatToShow == DTMFilter.SHOW_NAMESPACE)
      {
        ...
        if(isDownwardAxisOfMany(axis))
        {
          StepPattern attrPat = new StepPattern(whatToShow,
                                    pat.getNamespace(),
                                    pat.getLocalName(),
                                newAxis, 0); XNumber score = pat.getStaticScore();
            ...
            throws javax.xml.transform.TransformerException
  {
    ...
    if(null == ai)
    {
      whatToShow = compiler.getWhatToShow(opPos); ai = new StepPattern(whatToShow, compiler.getStepNS(opPos),
  ...
  public static final int BITMASK_TRAVERSES_OUTSIDE_SUBTREE = (BIT_NAMESPACE  | BIT_PRECEDING_SIBLING
                                                                | BIT_PRECEDING
                                                                | BIT_FOLLOWING_SIBLING
                                                                | BIT_FOLLOWING
                                                                | BIT_PARENT  | BIT_ANCESTOR_OR_SELF
                                                                | BIT_ANCESTOR
                                                                | BIT_FILTER
                                                                | BIT_ROOT);
