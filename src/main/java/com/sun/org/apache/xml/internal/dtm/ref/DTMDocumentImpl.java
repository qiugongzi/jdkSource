...
implements DTM, org.xml.sax.ContentHandler, org.xml.sax.ext.LexicalHandler
{
        ...
        int m_docHandle = NULL;          int m_docElement = NULL;         int currentParent = 0;                  int previousSibling = 0;                protected int m_currentNode = -1;               private boolean previousSiblingWasParent = false;
  ...
  private IncrementalSAXSource m_incrSAXSource=null;
        ...
        public DTMDocumentImpl(DTMManager mgr, int documentNumber,
                ...
                initDocument(documentNumber);    m_xsf = xstringfactory;
        }
  ...
  public void setIncrementalSAXSource(IncrementalSAXSource source)
  {
    ...
    source.setLexicalHandler(this);
        ...
        private final int appendNode(int w0, int w1, int w2, int w3)
        {
                ...
                previousSiblingWasParent = false;       return slotnumber;
        }
...
public boolean hasChildNodes(int nodeHandle) {
        ...
        {
                null,null,                                                      null,"#text",                                           "#cdata_section",null,  null,null,                                                      "#comment","#document", null,"#document-fragment", null};                                                                  public String getNodeName(int nodeHandle) {
                nodes.readSlot(nodeHandle, gotslot);
                short type = (short) (gotslot[0] & 0xFFFF);
        ...
        {
                nodes.readSlot(nodeHandle, gotslot);
                int nodetype=gotslot[0] & 0xFF;         String value=null;

                switch (nodetype) {                     case ATTRIBUTE_NODE:
                        nodes.readSlot(nodeHandle+1, gotslot);
                case TEXT_NODE:
                case COMMENT_NODE:
                case CDATA_SECTION_NODE:
                        value=m_char.getString(gotslot[2], gotslot[3]);         break;
                case PROCESSING_INSTRUCTION_NODE:
                case ELEMENT_NODE:
                case ENTITY_REFERENCE_NODE:
                default:
  ...
  void appendNSDeclaration(int prefixIndex, int namespaceIndex,
                           boolean isID)
  {
    final int namespaceForNamespaces=m_nsNames.stringToIndex("http:int w0 = NAMESPACE_NODE | (m_nsNames.stringToIndex("http:int w1 = currentParent;
    int w2 = 0;
    int w3 = namespaceIndex;
    int ourslot = appendNode(w0, w1, w2, w3);
    previousSibling = ourslot;  previousSiblingWasParent = false;
    return ;}
  ...
  void appendStartDocument()
  {
    ...
    m_docElement = NULL;         initDocument(0);
  }
