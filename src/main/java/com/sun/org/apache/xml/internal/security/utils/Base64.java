...
public class Base64 {
    ...
    static final byte[] getBytes(BigInteger big, int bitlen) {
        ...
        int startSrc = 0;    int bigLen = bigBytes.length;    if ((big.bitLength() % 8) == 0) {    startSrc = 1;    bigLen--;    }

        int startDst = bitlen / 8 - bigLen;    byte[] resizedBytes = new byte[bitlen / 8];
    ...
    protected static final byte[] decodeInternal(byte[] base64Data, int len)
        ...
        if ((b3 == -1) || (b4 == -1) ) {
            if (isPad(d3) && isPad(d4)) {               if ((b2 & 0xf) != 0) { throw new Base64DecodingException("decoding.general");
                }
                decodedData = new byte[encodedIndex + 1];
                decodedData[encodedIndex]   = (byte)(b1 << 2 | b2 >> 4) ;
            } else if (!isPad(d3) && isPad(d4)) {               if ((b3 & 0x3) != 0) { throw new Base64DecodingException("decoding.general");
                }
    ...
    protected static final void decode(byte[] base64Data, OutputStream os, int len)
        ...
        b3 = base64Alphabet[d3 = base64Data[dataIndex++]];
        b4 = base64Alphabet[d4 = base64Data[dataIndex++]];
        if ((b3 == -1 ) || (b4 == -1) ) { if (isPad(d3) && isPad(d4)) {               if ((b2 & 0xf) != 0) { throw new Base64DecodingException("decoding.general");
                }
                os.write((byte)(b1 << 2 | b2 >> 4));
            } else if (!isPad(d3) && isPad(d4)) {               if ((b3 & 0x3 ) != 0) { throw new Base64DecodingException("decoding.general");
                }
