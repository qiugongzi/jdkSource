...
CharInfo.getCharInfoInternal(CharInfo.HTML_ENTITIES_RESOURCE, Method.HTML);
    ...
    static void initTagReference(Trie m_elementFlags) {
        ...
        elemDesc.setAttr("SRC", ElemDesc.ATTRURL); elemDesc.setAttr("NOWRAP", ElemDesc.ATTREMPTY); elemDesc = (ElemDesc) m_elementFlags.get("DL");
        elemDesc.setAttr("COMPACT", ElemDesc.ATTREMPTY);
        ...
        elemDesc.setAttr("NOWRAP", ElemDesc.ATTREMPTY); elemDesc = (ElemDesc) m_elementFlags.get("TD");
            ...
            if ((ch < 32) || (ch > 126))
            {
                ...
                if (doURLEscaping)
                {
                    ...
                    else if (ch <= 0x7FF)
                    {
                        ...
                        int byte1 = 0xF0 | (uuuuu >> 2); int byte2 =
                    ...
                    {
                        int high = (ch >> 12) | 0xE0; int middle = ((ch & 0x0FC0) >> 6) | 0x80;
                        int low = (ch & 0x3F) | 0x80;
                        writer.write('%');
                        writer.write(makeHHString(high));
                        writer.write('%');
...
writeNormalizedChars(chars, start, length, false, m_lineSepUse);
        ...
        throws org.xml.sax.SAXException
    {
        ...
        else
        {
            ...
            writer.write(data); writer.write('>'); if (m_elemContext.m_currentElemDepth <= 0)
                outputLineSep();
