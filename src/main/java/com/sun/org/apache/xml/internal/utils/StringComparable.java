...
public class StringComparable implements Comparable  {
   ...
   if(m_text.equals(pattern)){return 0;
   }
   ...
   int comp = 0;
     ...
     }else{m_collator.setStrength(Collator.SECONDARY);
         comp = m_collator.compare(m_text, pattern );
     ...
     if(comp != 0){return comp ;
      ...
      comp = getCaseDiff(m_text, pattern);
       if(comp != 0){
           return comp;
       }else{return m_collator.compare(m_text, pattern );
       }
  }
  ...
  private final int getCaseDiff (final String text, final String pattern){
     ...
     m_collator.setStrength(Collator.TERTIARY);m_collator.setDecomposition(Collator.CANONICAL_DECOMPOSITION );final int diff[] =getFirstCaseDiff (text, pattern, m_locale);
    m_collator.setStrength(savedStrength);m_collator.setDecomposition(savedDecomposition); if(diff != null){
  ...
  private final int[] getFirstCaseDiff(final String text, final String pattern, final Locale locale){
        ...
        while (true) {
            ...
            if ((patternElement == done) ||( targetElement == done)) {
              ...
              getTarget = false;
            } else if (targetElement != patternElement) {if((startPatt < endPatt) && (startTarg < endTarg)){
                    ...
                    if(m_collator.compare(subTextUp, subPattUp) != 0){ continue;
                    }
