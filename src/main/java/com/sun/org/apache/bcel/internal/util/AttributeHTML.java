...
final class AttributeHTML implements com.sun.org.apache.bcel.internal.Constants {
  private String       class_name;     private PrintWriter  file;                                    private int          attr_count = 0;
  ...
  final void writeAttribute(Attribute attribute, String anchor, int method_number) throws IOException {
    ...
    if(tag == ATTR_UNKNOWN) return;

    attr_count++; if(attr_count % 2 == 0)
    ...
    case ATTR_CODE:
      ...
      if(len > 0) {
        ...
        for(int i=0; i < len; i++) {
          int catch_type = ce[i].getCatchType(); file.print("<LI>");

          if(catch_type != 0)
            file.print(constant_html.referenceConstant(catch_type)); else
    ...
    default: file.print("<P>" + attribute.toString());
    }
