...
public class MethodGen extends FieldGenOrMethodGen {
  ...
  public MethodGen(int access_flags, Type return_type, Type[] arg_types,
    ...
    if(!abstract_) {
      ...
      if(!isStatic() && (class_name != null)) { addLocalVariable("this", new ObjectType(class_name), start, end);
      }
    }
      ...
      if(a instanceof Code) {
        ...
        if(ces != null) {
          for(int j=0; j < ces.length; j++) {
            ...
            if(length == end_pc) { end = il.getEnd();
            } else {
  ...
  public LocalVariableGen addLocalVariable(String name, Type type, int slot,
    ...
    if(t != Constants.T_ADDRESS) {
      ...
      if((i = variable_vec.indexOf(l)) >= 0) variable_vec.set(i, l);
      else
        variable_vec.add(l);
  ...
  private static final void sort(LocalVariableGen[] vars, int l, int r) {
    ...
    do {
      ...
      if(i <= j) {
        h=vars[i]; vars[i]=vars[j]; vars[j]=h; i++; j--;
      }
    } while(i <= j);
  ...
  private LocalVariableGen addLocalVariableType(String name, Type type, int slot,
    ...
    if(t != Constants.T_ADDRESS) {
      ...
      if((i = type_vec.indexOf(l)) >= 0) type_vec.set(i, l);
      else
        type_vec.add(l);
  ...
  public LineNumberTable getLineNumberTable(ConstantPoolGen cp) {
    ...
    } catch(ArrayIndexOutOfBoundsException e) {} return new LineNumberTable(cp.addUtf8("LineNumberTable"),
  ...
  public Method getMethod() {
    ...
    int             exc_len = c_exc.length * 8; Code code = null;
    ...
    if((il != null) && !isAbstract()) {
      ...
      code = new Code(cp.addUtf8("Code"),
                      8 + byte_code.length + 2 + exc_len +          2 + attrs_len,         max_stack, max_locals,
                      byte_code, c_exc,
                      code_attrs,
                      cp.getConstantPool());
    ...
    if(throws_vec.size() > 0)
      addAttribute(et = getExceptionTable(cp)); Method m = new Method(access_flags, name_index, signature_index,
