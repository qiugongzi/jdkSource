...
public class InstructionList implements Serializable {
  ...
  private int               length = 0; private int[]             byte_positions; public InstructionList() {}
  ...
  public boolean isEmpty() { return start == null; } public static InstructionHandle findHandle(InstructionHandle[] ihs,
                                             int[] pos, int count,
                                             int target) {
    int l=0, r = count - 1;
    ...
    do {
      ...
      if(j == target) return ihs[i];
      else if(target < j) r = i - 1;
      else l = i + 1;
    } while(l <= r);
  ...
  public InstructionList(byte[] code) {
    ...
    int[]               pos   = new int[code.length]; int                 count = 0; try {
      while(bytes.available() > 0) {
        ...
        if(i instanceof BranchInstruction) ih = append((BranchInstruction)i);
        else
          ih = append(i);
    ...
    byte_positions = new int[count]; System.arraycopy(pos, 0, byte_positions, 0, count);
    ...
    for(int i=0; i < count; i++) {
      if(ihs[i] instanceof BranchHandle) {
        ...
        bi.setTarget(ih); if(bi instanceof Select) { Select s       = (Select)bi;
  ...
  public InstructionHandle append(InstructionHandle ih, InstructionList il) {
    ...
    if(next != null) next.prev = il.end;
    else
      end = il.end; length += il.length; il.clear();
  ...
  public InstructionHandle append(Instruction i, InstructionList il) {
    ...
    if((ih = findInstruction2(i)) == null) throw new ClassGenException("Instruction " + i +
  ...
  public InstructionHandle insert(InstructionHandle ih, InstructionList il) {
    ...
    if(prev != null) prev.next = il.start;
    else
      start = il.start; length += il.length; il.clear();
  ...
  public void move(InstructionHandle start, InstructionHandle end, InstructionHandle target) {
    ...
    for(InstructionHandle ih = start; ih != end.next; ih = ih.next) {
      if(ih == null) throw new ClassGenException("Invalid range: From " + start + " to " + end);
      else if(ih == target) throw new ClassGenException("Invalid range: From " + start + " to " + end +
                                    " contains target " + target);
    }
    ...
    if(target == null) { end.next = this.start;
      this.start = start;
    } else {
      next = target.next;
  ...
  {
    InstructionHandle first, last; if((prev == null) && (next == null)) { first = last = start;
      start = end = null;
    } else {
      if(prev == null) { first = start;
        start = next;
      } else {
        first     = prev.next;
        prev.next = next;
    ...
    for(InstructionHandle ih=first; ih != null; ih = ih.next)
      ih.getInstruction().dispose(); StringBuffer buf = new StringBuffer("{ ");
    for(InstructionHandle ih=first; ih != null; ih = next) {
      next = ih.next;
      length--;

      if(ih.hasTargeters()) { target_vec.add(ih);
  ...
  public void setPositions(boolean check) {
    ...
    if(check) {
      for(InstructionHandle ih=start; ih != null; ih = ih.next) {
        ...
        if(i instanceof BranchInstruction) { Instruction inst = ((BranchInstruction)i).getTarget().instruction;
          if(!contains(inst))
  ...
  public InstructionList copy() {
    ...
    for(InstructionHandle ih=start; ih != null; ih = ih.next) {
      Instruction i = ih.instruction;
      Instruction c = i.copy(); if(c instanceof BranchInstruction)
    ...
    while(ih != null) {
      ...
      if(i instanceof BranchInstruction) {
        ...
        InstructionHandle itarget = bi.getTarget(); bc.setTarget((InstructionHandle)map.get(itarget));

        if(bi instanceof Select) { InstructionHandle[] itargets = ((Select)bi).getTargets();
          InstructionHandle[] ctargets = ((Select)bc).getTargets();

          for(int j=0; j < itargets.length; j++) { ctargets[j] = (InstructionHandle)map.get(itargets[j]);
          }
        }
      }
  ...
  public void redirectBranches(InstructionHandle old_target,
    ...
    for(InstructionHandle ih = start; ih != null; ih = ih.next) {
      ...
      if(i instanceof BranchInstruction) {
        ...
        if(b instanceof Select) { InstructionHandle[] targets = ((Select)b).getTargets();

          for(int j=0; j < targets.length; j++) if(targets[j] == old_target)
