...
public class LOOKUPSWITCH extends Select {
  ...
  public void dump(DataOutputStream out) throws IOException {
    super.dump(out);
    out.writeInt(match_length);       for(int i=0; i < match_length; i++) {
      out.writeInt(match[i]);         out.writeInt(indices[i] = getTargetOffset(targets[i]));
    }
  }
  ...
  {
    super.initFromFile(bytes, wide); match_length = bytes.readInt();
