...
public final class DocumentCache implements DOMCache {
    ...
    private final long getLastModified(String uri) {
        try {
            ...
            if (timestamp == 0){ if ("file".equals(url.getProtocol())){
    ...
    if (baseURI != null && !baseURI.equals("")) {
        ...
        if ((doc = lookupDocument(uri)) == null) {
            ...
            if (doc == null) return null; doc.setLastModified(getLastModified(uri));
            insertDocument(uri, doc);
        }
        else {
        ...
        doc.incAccessCount(); final AbstractTranslet translet = (AbstractTranslet)trs;
