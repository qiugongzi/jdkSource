...
{ ER_DUPLICATE_VAR,
  ...
  { "noParsermsg5", "IBM's AlphaWorks: http:{ "optionURIRESOLVER", "   [-URIRESOLVER full class name (URIResolver to be used to resolve URIs)]"},
  ...
  {  "optionFLAVOR",  "   [-FLAVOR flavorName (Explicitly use s2s=SAX or d2d=DOM to do transform.)] "}, { "optionDIAG", "   [-DIAG (Print overall milliseconds transform took.)]"},
  { "optionINCREMENTAL",  "   [-INCREMENTAL (request incremental DTM construction by setting http:{  "optionNOOPTIMIMIZE",  "   [-NOOPTIMIMIZE (request no stylesheet optimization processing by setting http:{ "optionRL",  "   [-RL recursionlimit (assert numeric limit on stylesheet recursion depth.)]"},
