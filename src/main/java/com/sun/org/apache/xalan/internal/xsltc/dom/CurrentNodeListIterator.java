...
public final class CurrentNodeListIterator extends DTMAxisIteratorBase {
    ...
    public int next() {
        ...
        for (int index = _currentIndex; index < last; ) {
            ...
            final int node = _nodes.at(index++);        if (_filter.test(node, position, last, currentNode, translet,
                             this)) {
                _currentIndex = index;
    ...
    private int computePositionOfLast() {
        ...
        for (int index = _currentIndex; index < last; ) {
            final int position = _docOrder ? index + 1 : last - index;
            int nodeIndex = _nodes.at(index++);         if (_filter.test(nodeIndex, position, last, currNode, translet,
                             this)) {
                lastPosition++;
            }
        }
