    ...
    private Op compile(Token tok, Op next, boolean reverse) {
        ...
        case Token.NONGREEDYCLOSURE:
            ...
            if (max > 0) {
                ...
                } else {                        op = Op.createClosure(this.numberOfClosures++);
                }
                op.next = next;
...
public boolean matches(char[]  target) {
    ...
    void prepare() {
        ...
        if (this.operations != null
            ...
            else if (this.operations.getData() >= 0x10000) { this.fixedString = REUtil.decomposeToSurrogates(this.operations.getData());
            } else {
                char[] ac = new char[1];
    ...
    static final int USE_UNICODE_CATEGORY = 1<<5; static final int UNICODE_WORD_BOUNDARY = 1<<6; static final int PROHIBIT_HEAD_CHARACTER_OPTIMIZATION = 1<<7;
    ...
    private static final int getWordType0(char ch, int opts) {
        ...
        case Character.UPPERCASE_LETTER:      case Character.LOWERCASE_LETTER:      case Character.TITLECASE_LETTER:      case Character.MODIFIER_LETTER:       case Character.OTHER_LETTER:          case Character.LETTER_NUMBER:         case Character.DECIMAL_DIGIT_NUMBER:  case Character.OTHER_NUMBER:          case Character.COMBINING_SPACING_MARK: return WT_LETTER;

        case Character.FORMAT:                case Character.NON_SPACING_MARK:      case Character.ENCLOSING_MARK:        return WT_IGNORE;

        case Character.CONTROL:               switch (ch) {
            case '\t':
            case '\n':
            case '\u000B':
            case '\f':
    ...
    private static final boolean isWordChar(int ch) { if (ch == '_')  return true;
