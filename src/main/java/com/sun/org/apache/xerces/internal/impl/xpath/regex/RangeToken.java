...
final class RangeToken extends Token implements java.io.Serializable {
    ...
    protected void compactRanges() {
        ...
        if (this.isCompacted())
            return;
        int base = 0;                           int target = 0;                         while (target < this.ranges.length) {
            if (base != target) {
    ...
    protected void subtractRanges(Token token) {
        ...
        while (src < this.ranges.length && sub < tok.ranges.length) {
            ...
            if (srcend < subbegin) {            result[wp++] = this.ranges[src++];
                result[wp++] = this.ranges[src++];
            } else if (srcend >= subbegin
                       && srcbegin <= subend) { if (subbegin <= srcbegin && srcend <= subend) {
                                                src += 2;
    ...
    protected void intersectRanges(Token token) {
        ...
        while (src1 < this.ranges.length && src2 < tok.ranges.length) {
            ...
            if (src1end < src2begin) {          src1 += 2;
            } else if (src1end >= src2begin
                       && src1begin <= src2end) { if (src2begin <= src2begin && src1end <= src2end) {
                                                result[wp++] = src1begin;
                    result[wp++] = src1end;
                    src1 += 2;
    ...
    private void createMap() {
        int asize = MAPSIZE/32;                 int [] map = new int[asize];
