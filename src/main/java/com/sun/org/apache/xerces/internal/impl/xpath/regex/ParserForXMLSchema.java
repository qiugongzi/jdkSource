...
class ParserForXMLSchema extends RegexParser {
    ...
    protected RangeToken parseCharacterClass(boolean useNrange) throws ParseException {
        ...
        boolean wasDecoded = false;                     RangeToken base = null;
        RangeToken tok;
        if (this.read() == T_CHAR && this.chardata == '^') {
            nrange = true;
            this.next();                        base = Token.createRange();
        ...
        while ((type = this.read()) != T_EOF) { wasDecoded = false;
            ...
            if (type == T_BACKSOLIDUS) {
                ...
                break;                          }
            this.next();
            if (!end) {                         if (type == T_CHAR) {
                    if (c == '[')  throw this.ex("parser.cc.6", this.offset-2);
                    if (c == ']')  throw this.ex("parser.cc.7", this.offset-2);
                    if (c == '-' && this.chardata != ']' && !firstloop)  throw this.ex("parser.cc.8", this.offset-2);   }
                if (this.read() != T_CHAR || this.chardata != '-' || c == '-' && firstloop) { if (!this.isSet(RegularExpression.IGNORE_CASE) || c > 0xffff) {
                        tok.addRange(c, c);
                    }
                    else {
                        addCaseInsensitiveChar(tok, c);
                    }
                } else {                        this.next(); if ((type = this.read()) == T_EOF)  throw this.ex("parser.cc.2", this.offset);
                                                if(type == T_CHAR && this.chardata == ']') {                                if (!this.isSet(RegularExpression.IGNORE_CASE) || c > 0xffff) {
                            tok.addRange(c, c);
                        }
                        else {
                            addCaseInsensitiveChar(tok, c);
