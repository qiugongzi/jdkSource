...
class Token implements java.io.Serializable {
    ...
    final int analyzeFirstCharacter(RangeToken result, int options) {
        switch (this.type) {
          ...
          case NRANGE:                          result.mergeRanges(Token.complementRanges(this));
            return FC_TERMINAL;
          ...
          case BACKREFERENCE:
            result.addRange(0, UTF16_MAX);  return FC_ANY;
    ...
    final void findFixedString(FixedStringContainer container, int options) {
        switch (this.type) {
          ...
          case CHAR:                            container.token = null;             return;                             case STRING:
    ...
    private static final String[] categoryNames = {
        ...
        "Pd", "Ps", "Pe", "Pc", "Po", "Sm", "Sc", "Sk", "So", "Pi", "Pf",  "L", "M", "N", "Z", "C", "P", "S",      };

    static final int CHAR_INIT_QUOTE  = 29;     static final int CHAR_FINAL_QUOTE = 30;     static final int CHAR_LETTER = 31;
    ...
    static final int[] nonBMPBlockRanges = {
        0x10300, 0x1032F,       0x10330, 0x1034F,
        0x10400, 0x1044F,
        0x1D000, 0x1D0FF,
        0x1D100, 0x1D1FF,
        0x1D400, 0x1D7FF,
            ...
            synchronized (Token.categories) {
                ...
                for (int i = 0;  i < ranges.length;  i ++) {
                    if (Token.categoryNames[i] != null) {
                        if (i == Character.UNASSIGNED) { ranges[i].addRange(0x10000, Token.UTF16_MAX);
                        }
                ...
                isalpha.mergeRanges(ranges[Character.UPPERCASE_LETTER]); isalpha.mergeRanges(ranges[Character.LOWERCASE_LETTER]); isalpha.mergeRanges(ranges[Character.OTHER_LETTER]); Token.categories.put("IsAlpha", isalpha);
                Token.categories2.put("IsAlpha", Token.complementRanges(isalpha));
                Token.registerNonXS("IsAlpha");

                Token isalnum = Token.createRange();
                isalnum.mergeRanges(isalpha);   isalnum.mergeRanges(ranges[Character.DECIMAL_DIGIT_NUMBER]); Token.categories.put("IsAlnum", isalnum);
                ...
                isspace.mergeRanges(ranges[CHAR_SEPARATOR]); Token.categories.put("IsSpace", isspace);
                Token.categories2.put("IsSpace", Token.complementRanges(isspace));
                Token.registerNonXS("IsSpace");

                Token isword = Token.createRange();
                isword.mergeRanges(isalnum);     isword.addRange('_', '_');
                ...
                Token.setAlias("word", "IsWord", true); Token.setAlias("xdigit", "IsXDigit", true);
    ...
    "\u094D"+"\u09CD"+"\u0A4D"+"\u0ACD"+"\u0B4D"+"\u0BCD"+"\u0C4D"+"\u0CCD"+"\u0D4D"+"\u0E3A"+"\u0F84";static private Token token_grapheme = null;
    static synchronized Token getGraphemePattern() {
        if (Token.token_grapheme != null)
            return Token.token_grapheme;

        Token base_char = Token.createRange();  base_char.mergeRanges(Token.getRange("ASSIGNED", true));
        ...
        combiner_wo_virama.addRange(0x1160, 0x11ff); combiner_wo_virama.addRange(0xff9e, 0xff9f); Token left = Token.createUnion();       left.addChild(base_char);
    ...
    static synchronized Token getCombiningCharacterSequence() {
        ...
        Token foo = Token.createClosure(Token.getRange("M", true)); foo = Token.createConcat(Token.getRange("M", false), foo); Token.token_ccs = foo;
        return Token.token_ccs;
    }
        ...
        void addChild(Token tok) {
            ...
            if (this.type == UNION) {
                ...
                for (int i = 0;  i < tok.size();  i ++)
                    this.addChild(tok.getChild(i)); return;
            }
            ...
            if (previous.type == CHAR) {        buffer = new StringBuilder(2 + nextMaxLength);
            ...
            } else {                            buffer = new StringBuilder(previous.getString().length() + nextMaxLength);
