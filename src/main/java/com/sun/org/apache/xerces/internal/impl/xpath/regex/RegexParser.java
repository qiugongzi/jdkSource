...
class RegexParser {
          ...
          case '(':
            ...
            switch (ch) {
              ...
              case '#':
                ...
                break;
              default:
                if (ch == '-' || 'a' <= ch && ch <= 'z' || 'A' <= ch && ch <= 'Z') {this.offset --;
                    ret = T_MODIFIERS;
                    break;
                } else if (ch == '(') {         ret = T_CONDITION;          break;
                }
    ...
    Token processBacksolidus_c() throws ParseException {
        int ch2;                                if (this.offset >= this.regexlen
        ...
        switch (ch) {
          ...
          case T_CHAR:
            if (this.chardata == '{' && this.offset < this.regexlen) {
                ...
                int off = this.offset;          int min = 0, max = -1;
                ...
                if (ch == ',') {
                   ...
                   if (off >= this.regexlen) {
                       throw ex("parser.quantifier.3", this.offset);
                        ...
                        max = ch -'0';       while (off < this.regexlen
               ...
               if (this.checkQuestion(off)) {  tok = Token.createNGClosure(tok);
                    this.offset = off+1;
                } else {
    ...
    Token parseAtom() throws ParseException {
        ...
        switch (ch) {
          ...
          case T_LPAREN2:       return this.processParen2(); case T_CONDITION:     return this.processCondition(); case T_MODIFIERS:     return this.processModifiers(); case T_INDEPENDENT:   return this.processIndependent();
          case T_DOT:
            this.next();                    tok = Token.token_dot;
            break;
    ...
    protected RangeToken parseCharacterClass(boolean useNrange) throws ParseException {
        ...
        this.next();                            boolean nrange = false;
        RangeToken base = null;
        RangeToken tok;
        if (this.read() == T_CHAR && this.chardata == '^') {
            nrange = true;
            this.next();                        if (useNrange) {
        ...
        while ((type = this.read()) != T_EOF) {
            ...
            if (type == T_BACKSOLIDUS) {
                ...
                break;                          }
            this.next();
            if (!end) {                         if (this.read() != T_CHAR || this.chardata != '-') { if (!this.isSet(RegularExpression.IGNORE_CASE) || c > 0xffff) {
                        tok.addRange(c, c);
                    }
                    else {
                    ...
                    this.next(); if ((type = this.read()) == T_EOF)  throw this.ex("parser.cc.2", this.offset);
    ...
    int decodeEscaped() throws ParseException {
        ...
        int c = this.chardata;
        switch (c) {
          case 'e':  c = 0x1b;  break; case 'f':  c = '\f';  break; case 'n':  c = '\n';  break; case 'r':  c = '\r';  break; case 't':  c = '\t';  break; case 'x':
            this.next();
