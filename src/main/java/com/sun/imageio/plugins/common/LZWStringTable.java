...
public class LZWStringTable {
    ...
    byte[]  strChr;  short[] strNxt;  short[] strHsh;  short numStrings;  int[] strLen;
    ...
    public short findCharString(short index, byte b) {
        ...
        hshidx = hash(index, b);
        while ((nxtidx = strHsh[hshidx]) != HASH_FREE) { if (strNxt[nxtidx] == index && strChr[nxtidx] == b) {
                return (short)nxtidx;
            }
            hshidx = (hshidx + HASHSTEP) % HASHSIZE;
        }
    ...
    public int expandCode(byte[] buf, int offset, short code, int skipHead) {
        ...
        int expandLen;  int codeLen = strLen[code] - skipHead; int bufSpace = buf.length - offset;  if (bufSpace > codeLen) {
            expandLen = codeLen; } else {
            expandLen = bufSpace;
        }

        int skipTail = codeLen - expandLen;  int idx = offset + expandLen;   while ((idx > offset) && (code != (short)0xFFFF)) {
            if (--skipTail < 0) { buf[--idx] = strChr[code];
            }
            code = strNxt[code];    }

        if (codeLen > expandLen) {
            return -expandLen; } else {
            return expandLen;     }
    }
