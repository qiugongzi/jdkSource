...
public class BitFile {
    ...
    byte buffer[];
    int index;
    int bitsLeft; boolean blocks = false;
    ...
    public void writeBits(int bits, int numbits) throws IOException {
        ...
        int numBytes = 255;  do {
            ...
            if (numbits <= bitsLeft) { if (blocks) { buffer[index] |= (bits & ((1 << numbits) - 1)) << (8 - bitsLeft);
                    bitsWritten += numbits;
                    bitsLeft -= numbits;
                    numbits = 0;
                } else {
            ...
            } else { if (blocks) { buffer[index] |= (bits & ((1 << bitsLeft) - 1)) << (8 - bitsLeft);
                    ...
                    numbits -= bitsLeft;  bitsWritten += bitsLeft;
                    buffer[++index] = 0;  bitsLeft = 8;
                }
            }
        } while (numbits != 0);
    }
