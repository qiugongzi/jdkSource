...
public class JPEGImageWriter extends ImageWriter {
    ...
    private boolean convertTosRGB = false;  private WritableRaster converted = null;
    ...
    private boolean ignoreJFIF = false;  private boolean forceJFIF = false;  private boolean ignoreAdobe = false;  private int newAdobeTransform = JPEG.ADOBE_IMPOSSIBLE;  private boolean writeDefaultJFIF = false;
    ...
    public void setOutput(Object output) {
        ...
        try {
            ...
            super.setOutput(output); resetInternalState();
            ios = (ImageOutputStream) output; setDest(structPointer);
        } finally {
            clearThreadLock();
        }
    }
    ...
    private void writeOnThread(IIOMetadata streamMetadata,
        ...
        ignoreJFIF = false;  ignoreAdobe = false;  newAdobeTransform = JPEG.ADOBE_IMPOSSIBLE;  writeDefaultJFIF = false;
        writeAdobe = false;
        ...
        iccProfile = null;  convertTosRGB = false;  converted = null;
        ...
        if (destType != null) {
            ...
            } else { if (JPEG.isJFIFcompliant(destType, false)) {
            ...
            outCsType = getDefaultDestCSType(destType);
                ...
                if (fullImage) {  metadata = new JPEGMetadata(new ImageTypeSpecifier(rimage),
                                                param, this);
                    if (metadata.findMarkerSegment
                        (JFIFMarkerSegment.class, true) != null) {
                ...
                } else { checkSOFBands(sof, numBandsUsed);
                if (fullImage) {  ImageTypeSpecifier inputType =
        ...
        if (metadata != null) {
            ...
            if ((sof != null) && (sof.tag == JPEG.SOF2)) {
                ...
                if (progressiveMode == ImageWriteParam.MODE_COPY_FROM_METADATA) {
                    scans = collectScans(metadata, sof);  } else {
                    numScans = 0;
                }
            }
            if (jfif == null) {
        ...
        if (!writeDefaultJFIF) {
            if (metadata == null) {
                ...
                if (fullImage == false) {
                    if (jfif == null) {
                        thumbnails = null;  if (numThumbs != 0) {
                            warningOccurred(WARNING_IGNORING_THUMBS);
                        }
                    }
                } else {  if (jfif == null) {  if ((outCsType == JPEG.JCS_GRAYSCALE)
                            || (outCsType == JPEG.JCS_YCbCr)) {
                            if (numThumbs != 0) {
                                forceJFIF = true;
                                warningOccurred(WARNING_FORCING_JFIF);
                            }
                        } else {  thumbnails = null;
                            if (numThumbs != 0) {
                                warningOccurred(WARNING_IGNORING_THUMBS);
                            }
                        }
        ...
        for (int i = 0; i < numBandsUsed; i++) {
            componentIds[i] = i+1; HsamplingFactors[i] = 1;
        ...
        if (sof != null) {
            for (int i = 0; i < numBandsUsed; i++) {
                if (forceJFIF == false) {  componentIds[i] = sof.componentSpecs[i].componentId;
                }
    ...
    private void checkJFIF(JFIFMarkerSegment jfif,
        ...
        if (jfif != null) {
            if (!JPEG.isJFIFcompliant(type, input)) {
                ignoreJFIF = true;  warningOccurred(input
                                ? WARNING_IMAGE_METADATA_JFIF_MISMATCH
    ...
    private int [] collectScans(JPEGMetadata metadata,
        ...
        if (!segments.isEmpty()) {
            ...
            for (int i = 0; i < numScans; i++) {
                ...
                retval[index++] = sos.componentSpecs.length; for (int j = 0; j < MAX_COMPS_PER_SCAN; j++) {
