...
public class JPEGImageReader extends ImageReader {
    ...
    private static final int UNKNOWN = -1;  private static final int MIN_ESTIMATED_PASSES = 10; private int knownPassCount = UNKNOWN;
    private int pass = 0;
                         ...
                         boolean ignoreMetadata)
    {
        ...
        try {
            ...
            resetInternalState();
            iis = (ImageInputStream) input; setSource(structPointer);
        } finally {
            clearThreadLock();
        }
    }
    ...
    private void checkTablesOnly() throws IOException {
        ...
        } else { imagePositions.add(new Long(savePos));
            currentImage = 0;
        }
        if (seekForwardOnly) {
        ...
        if (allowSearch == true) {
            ...
            while (!done) {
                ...
                case JPEG.EOI:
                    ...
                    buffer.bufAvail -= 2;
                    length -= 2; buffer.skipData(length);
                }
            }
    ...
    private void readHeader(int imageIndex, boolean reset)
        throws IOException {
        gotoImage(imageIndex);
        readNativeHeader(reset); currentImage = imageIndex;
    }
    ...
    private Iterator getImageTypesOnThread(int imageIndex)
        ...
        case JPEG.JCS_YCbCr:
                ...
                list.add(new ImageTypeProducer() {
                    protected ImageTypeSpecifier produce() {
                        return ImageTypeSpecifier.createInterleaved
                         (iccCS,
                          JPEG.bOffsRGB,  DataBuffer.TYPE_BYTE,
                          false,
                          false);
                    }
                });
            ...
            list.add(getImageType(JPEG.JCS_YCC));
            break;
        case JPEG.JCS_YCbCrA:  list.add(getImageType(JPEG.JCS_RGBA));
            break;
        }
    ...
    private void checkColorConversion(BufferedImage image,
        ...
        case JPEG.JCS_GRAYSCALE:  if  (csType == ColorSpace.TYPE_RGB) { setOutColorSpace(structPointer, JPEG.JCS_RGB);
            ...
            break;
        case JPEG.JCS_RGB:  if (csType ==  ColorSpace.TYPE_GRAY) {  if (colorSpaceCode == JPEG.JCS_YCbCr) {
        ...
        case JPEG.JCS_YCC:
            {
                ...
                if (YCC == null) { throw new IIOException("Incompatible color conversion");
                }
                if ((cs != YCC) &&
                    (cm.getNumComponents() == numComponents)) {
    ...
    private Raster readInternal(int imageIndex,
        ...
        if (param != null) {
            ...
            if (!wantRaster) {  destinationBands = param.getDestinationBands();
            }
        ...
        initProgressData();
        if (imageIndex == imageMetadataIndex) { knownPassCount = 0;
    ...
    public int getNumThumbnails(int imageIndex) throws IOException {
        ...
        try {
            ...
            getImageMetadata(imageIndex);  JFIFMarkerSegment jfif =
