...
class JPEGBuffer {
    ...
    void loadBuf(int count) throws IOException {
        ...
        if (count != 0) {
            if (bufAvail >= count) {  return;
            }
        } else {
            if (bufAvail == BUFFER_SIZE) {  return;
            }
        }
    ...
    void readData(byte [] data) throws IOException {
        ...
        if (bufAvail >= count) {  System.arraycopy(buf, bufPtr, data, 0, count);
            bufAvail -= count;
            bufPtr += count;
            return;
        }
        int offset = 0;
        if (bufAvail > 0) {  System.arraycopy(buf, bufPtr, data, 0, bufAvail);
            offset = bufAvail;
            count -= bufAvail;
            bufAvail = 0;
            bufPtr = 0;
    ...
    void skipData(int count) throws IOException {
        if (bufAvail >= count) {  bufAvail -= count;
            bufPtr += count;
            return;
        }
        if (bufAvail > 0) {  count -= bufAvail;
            bufAvail = 0;
            bufPtr = 0;
        }
