...
class JFIFMarkerSegment extends MarkerSegment {
    ...
    int thumbWidth;
    int thumbHeight;
    JFIFThumbRGB thumb = null;  ArrayList extSegments = new ArrayList();
    ICCMarkerSegment iccSegment = null; private static final int THUMB_JPEG = 0x10;
    ...
    JFIFMarkerSegment(JPEGBuffer buffer) throws IOException {
        super(buffer);
        buffer.bufPtr += ID_SIZE;  majorVersion = buffer.buf[buffer.bufPtr++];
    ...
    protected Object clone() {
        ...
        if (!extSegments.isEmpty()) { newGuy.extSegments = new ArrayList();
    ...
    void addICC(JPEGBuffer buffer) throws IOException {
        if (inICC == false) {
            ...
            if (inICC == false) { iccSegment = tempICCSegment;
                tempICCSegment = null;
            }
        } else {
    ...
    void writeThumbnailData(ImageOutputStream ios,
                            int [] thumbData,
                            JPEGImageWriter writer) throws IOException {
        int progInterval = thumbData.length / 20;  if (progInterval == 0) {
            progInterval = 1;
        }
    ...
    void writeWithThumbs(ImageOutputStream ios,
        ...
        if (thumbnails != null) {
            ...
            if (thumbnails.size() == 1) {
                ...
                write(ios, writer);  for (int i = 0; i < thumbnails.size(); i++) {
                    jfxx = null;
    ...
    private void writeThumb(ImageOutputStream ios,
        ...
        if (cm instanceof IndexColorModel) {
            ...
            if ((jfxx == null)
                ...
                writeJFXXSegment(index, thumb, ios, writer); } else {
                ...
                jfxx.write(ios, writer);  writer.thumbnailComplete();
            }
        } else if (cs.getType() == ColorSpace.TYPE_RGB) {
            if (jfxx == null) {
                if (onlyOne) {
                    write(ios, thumb, writer); } else {
                ...
                if (jfxx.code == THUMB_PALETTE) {
                    writeJFXXSegment(index, thumb, ios, writer); writer.warningOccurred
            ...
            if (jfxx == null) {
                if (onlyOne) {
                    ...
                    write(ios, thumbRGB, writer); } else {
                    writeJFXXSegment(index, thumb, ios, writer); }
            } else {
                if (onlyOne) {
                    write(ios, writer);
    ...
    class JFIFExtensionMarkerSegment extends MarkerSegment {
        ...
        JFIFExtensionMarkerSegment(JPEGBuffer buffer, JPEGImageReader reader)
            ...
            super(buffer);
            buffer.bufPtr += ID_SIZE;  code = buffer.buf[buffer.bufPtr++] & 0xff;
            buffer.bufAvail -= DATA_SIZE;
            if (code == THUMB_JPEG) {
    ...
    abstract class JFIFThumb implements Cloneable {
        long streamPos = -1L;  abstract int getLength(); abstract int getWidth();
    ...
    class JFIFThumbRGB extends JFIFThumbUncompressed {
        ...
        void write(ImageOutputStream ios,
                   JPEGImageWriter writer) throws IOException {
            super.write(ios, writer); writePixels(ios, writer);
        }
    ...
    class JFIFThumbPalette extends JFIFThumbUncompressed {
        ...
        void write(ImageOutputStream ios,
                   JPEGImageWriter writer) throws IOException {
            super.write(ios, writer); byte [] palette = new byte[768];
            IndexColorModel icm = (IndexColorModel) thumbnail.getColorModel();
    ...
    class JFIFThumbJPEG extends JFIFThumb {
        ...
        byte [] data = null;  private static final int PREAMBLE_SIZE = 6;
        ...
        void write(ImageOutputStream ios,
                   JPEGImageWriter writer) throws IOException {
            int progInterval = data.length / 20;  if (progInterval == 0) {
                progInterval = 1;
            }
            for (int offset = 0;
                 offset < data.length;) {
    ...
    static void writeICC(ICC_Profile profile, ImageOutputStream ios)
        ...
        int ID_LENGTH = ID.length()+1; int COUNTS_LENGTH = 2;
        ...
        for (int i = 0; i < numChunks; i++) {
            ...
            ios.write(id);
            ios.write(0); ios.write(chunkNum++);
            ios.write(numChunks);
    ...
    class ICCMarkerSegment extends MarkerSegment {
        ...
        byte [] profile = null; private static final int ID_SIZE = 12;
        int chunksRead;
        int numChunks;
        ...
        ICCMarkerSegment(JPEGBuffer buffer) throws IOException {
            super(buffer);  if (debug) {
                System.out.println("Creating new ICC segment");
            }
            buffer.bufPtr += ID_SIZE; buffer.bufAvail -= ID_SIZE;
        ...
        ICCMarkerSegment(Node node) throws IIOInvalidTreeException {
            ...
            if (node instanceof IIOMetadataNode) {
                ...
                if (prof != null) {  profile = prof.getData();
                }
            }
        }
            ...
            if (chunksRead < numChunks) {
                ...
                for (int i = 1; i <= numChunks; i++) {
                    ...
                    for (int chunk = 0; chunk < chunks.size(); chunk++) {
                        ...
                        if (chunkData[0] == i) { System.arraycopy(chunkData, 2,
                                             profile, index,
                                             chunkData.length-2);
