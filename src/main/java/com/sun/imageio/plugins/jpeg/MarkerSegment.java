...
class MarkerSegment implements Cloneable {
    protected static final int LENGTH_SIZE = 2; int tag;      int length;
    byte [] data = null;  boolean unknown = false; MarkerSegment(JPEGBuffer buffer) throws IOException {

        buffer.loadBuf(3);  tag = buffer.buf[buffer.bufPtr++] & 0xff;
        length = (buffer.buf[buffer.bufPtr++] & 0xff) << 8;
        length |= buffer.buf[buffer.bufPtr++] & 0xff;
        length -= 2;  if (length < 0) {
    ...
    protected Object clone() {
        ...
        } catch (CloneNotSupportedException e) {} if (this.data != null) {
