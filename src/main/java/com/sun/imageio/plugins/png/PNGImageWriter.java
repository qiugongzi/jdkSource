...
final class IDATOutputStream extends ImageOutputStreamImpl {
    ...
    private void startChunk() throws IOException {
        crc.reset();
        this.startPos = stream.getStreamPosition();
        stream.writeInt(-1); crc.update(chunkType, 0, 4);
...
public class PNGImageWriter extends ImageWriter {
    ...
    int[] sampleSize = null; int scalingBitDepth = -1; byte[][] scale = null; byte[] scale0 = null; byte[][] scaleh = null; byte[][] scalel = null; int totalPixels; int pixelsDone; public PNGImageWriter(ImageWriterSpi originatingProvider) {
...
"Only interlace methods 0 (node) and 1 (adam7) are defined in PNG 1.1");
    ...
    private void write_iCCP() throws IOException {
        if (metadata.iCCP_present) {
            ...
            cs.writeByte(0); cs.writeByte(metadata.iCCP_compressionMethod);
...
"The chunk will not be written.");
    ...
    private void write_sPLT() throws IOException {
        if (metadata.sPLT_present) {
            ...
            cs.writeByte(0); cs.writeByte(metadata.sPLT_sampleDepth);
