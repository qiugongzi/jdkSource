...
public class GIFImageReader extends ImageReader {
    ...
    private void readMetadata() throws IIOException {
        ...
        try {
            ...
            while (true) {
                int blockType = stream.readUnsignedByte();
                if (blockType == 0x2c) { imageMetadata.imageLeftPosition =
                    ...
                    return;
                } else if (blockType == 0x21) { int label = stream.readUnsignedByte();

                    if (label == 0xf9) { int gceLength = stream.readUnsignedByte(); int gcePackedFields = stream.readUnsignedByte();
                    ...
                    } else if (label == 0x1) { int length = stream.readUnsignedByte();
                    ...
                    } else if (label == 0xfe) { byte[] comment = concatenateBlocks();
                        if (imageMetadata.comments == null) {
                            imageMetadata.comments = new ArrayList();
                        }
                        imageMetadata.comments.add(comment);
                    } else if (label == 0xff) { int blockSize = stream.readUnsignedByte();
                ...
                } else if (blockType == 0x3b) { throw new IndexOutOfBoundsException
