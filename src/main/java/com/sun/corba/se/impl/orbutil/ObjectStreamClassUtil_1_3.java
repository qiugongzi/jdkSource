...
public final class ObjectStreamClassUtil_1_3 {
    ...
    public static long computeSerialVersionUID(final Class cl) {
        ...
        if (csuid == 0)
            return csuid; csuid = (ObjectStreamClassUtil_1_3.getSerialVersion(csuid, cl).longValue());
        return csuid;
    }
    ...
    private static long _computeSerialVersionUID(Class cl) {
        ...
        try {
            ...
            if (hasStaticInitializer(cl)) {
                ...
                data.writeInt(Modifier.STATIC); data.writeUTF("()V");
            }
    ...
    private static String getSignature(Constructor cons) {
        ...
        Class[] params = cons.getParameterTypes(); for (int j = 0; j < params.length; j++) {
            sb.append(getSignature(params[j]));
        }
        sb.append(")V");
        return sb.toString();
    ...
    private static class MethodSignature implements Comparator {
        Member member;
        String signature;      static MethodSignature[] removePrivateAndSort(Member[] m) {
            int numNonPrivate = 0;
        ...
        public int compare(Object o1, Object o2) {
            ...
            } else { result = c1.member.getName().compareTo(c2.member.getName());
                if (result == 0)
                    result = c1.signature.compareTo(c2.signature);
            }
            return result;
