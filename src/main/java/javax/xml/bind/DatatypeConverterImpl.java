...
break;
    ...
    public static QName _parseQName(CharSequence text, NamespaceContext nsc) {
        ...
        int idx = start + 1;    while (idx < end && text.charAt(idx) != ':') {
            idx++;
        }
...
return val;
    ...
    public static String _printQName(QName val, NamespaceContext nsc) {
        ...
        if (prefix == null || prefix.length() == 0) { qname = localPart;
        } else {
    ...
    private static final class CalendarFormatter {
        ...
        public static String doFormat(String format, Calendar cal) throws IllegalArgumentException {
            ...
            while (fidx < flen) {
                ...
                if (fch != '%') {  buf.append(fch);
                    continue;
                }

                switch (format.charAt(fidx++)) {
                    case 'Y': formatYear(cal, buf);
                        break;

                    case 'M': formatMonth(cal, buf);
                        break;

                    case 'D': formatDays(cal, buf);
                        break;

                    case 'h': formatHours(cal, buf);
                        break;

                    case 'm': formatMinutes(cal, buf);
                        break;

                    case 's': formatSeconds(cal, buf);
                        break;

                    case 'z': formatTimeZone(cal, buf);
                        break;
        ...
        private static void formatSeconds(Calendar cal, StringBuilder buf) {
            ...
            if (cal.isSet(Calendar.MILLISECOND)) { int n = cal.get(Calendar.MILLISECOND);
                if (n != 0) {
        ...
        private static void formatTimeZone(Calendar cal, StringBuilder buf) {
            ...
            offset /= 60 * 1000; formatTwoDigits(offset / 60, buf);
            buf.append(':');
