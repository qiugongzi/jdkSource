...
public abstract class ImageOutputStreamImpl
    ...
    public void writeBits(long bits, int numBits) throws IOException {
        ...
        if ((getStreamPosition() > 0) || (bitOffset > 0)) {
            int offset = bitOffset;  int partialByte = read();
            if (partialByte != -1) {
            ...
            if (numBits + offset < 8) {
                ...
                partialByte &= ~(mask << shift);  partialByte |= ((bits & mask) << shift); write(partialByte);
                seek(getStreamPosition() - 1);
                bitOffset = offset + numBits;
                numBits = 0;  } else {
                int num = 8 - offset;
                int mask = -1 >>> (32 - num);
                partialByte &= ~mask;  partialByte |= ((bits >> (numBits - num)) & mask);
                write(partialByte);
                numBits -= num;
            }
        }
    ...
    protected final void flushBits() throws IOException {
        ...
        if (bitOffset != 0) {
            ...
            int partialByte = read(); if (partialByte < 0) {
                partialByte = 0;
                bitOffset = 0;
            }
            else {
