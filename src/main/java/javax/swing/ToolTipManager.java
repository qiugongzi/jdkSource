...
;
    ...
    public void mouseExited(MouseEvent event) {
        ...
        if (window != null && event.getSource() == window && insideComponent != null) {
          Container insideComponentWindow = insideComponent.getTopLevelAncestor();
            ...
            if (win != null) {  Point location = SwingUtilities.convertPoint(insideComponent,
                                                             event.getPoint(),
                                                             win);
    ...
    if (invoker != null){
      ...
      for (parent = invoker.getParent(); parent != null; parent = parent.getParent()){
        if(parent instanceof JFrame || parent instanceof JDialog ||
           parent instanceof JWindow) { return getWidthAdjust(parent.getBounds(),popupRectInScreen);
  ...
  private int getWidthAdjust(Rectangle a, Rectangle b){
    ...
    private void show(JComponent source) {
        if (tipWindow != null) { hideTipWindow();
            insideComponent = null;
        }
        else {
            hideTipWindow(); enterTimer.stop();
            exitTimer.stop();
            insideTimer.stop();
