...
class InnerShadowEffect extends ShadowEffect {
    ...
    BufferedImage applyEffect(BufferedImage src, BufferedImage dst, int w, int h) {
        ...
        EffectUtils.blur(srcAlphaBuf, tmpBuf2, tmpW, tmpH, kernel, size * 2); EffectUtils.blur(tmpBuf2, tmpBuf1, tmpH, tmpW, kernel, size * 2);float spread = Math.min(1 / (1 - (0.01f * this.spread)), 255);
