...
class DropShadowEffect extends ShadowEffect {
    ...
    BufferedImage applyEffect(BufferedImage src, BufferedImage dst, int w, int h) {
        ...
        EffectUtils.blur(tmpBuf1, tmpBuf2, tmpW, tmpH, kernel, size); EffectUtils.blur(tmpBuf2, tmpBuf1, tmpH, tmpW, kernel, size);float spread = Math.min(1 / (1 - (0.01f * this.spread)), 255);
