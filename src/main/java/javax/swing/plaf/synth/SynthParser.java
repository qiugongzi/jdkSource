...
class SynthParser extends DefaultHandler {
    ...
    private void startProperty(Attributes attributes,
        ...
        if (aValue != null) {
            switch (iType) {
            case 0: value = lookup(aValue, Object.class);
                break;
            case 1: if (aValue.toUpperCase().equals("TRUE")) {
                    value = Boolean.TRUE;
                }
                else {
                    value = Boolean.FALSE;
                ...
                break;
            case 2: StringTokenizer tok = new StringTokenizer(aValue);
                value = new DimensionUIResource(
                    nextInt(tok, "Invalid dimension"),
                    nextInt(tok, "Invalid dimension"));
                break;
            case 3: value = parseInsets(aValue, property + " invalid insets");
                break;
            case 4: try {
