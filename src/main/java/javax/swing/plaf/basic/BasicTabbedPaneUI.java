...
public void paint(Graphics g, JComponent c) {
    ...
    private static Polygon createCroppedTabShape(int tabPlacement, Rectangle tabRect, int cropline) {
        ...
        } else { return new Polygon(yp, xp, pcnt);
        }
    }
    ...
    protected void paintText(Graphics g, int tabPlacement,
        ...
        if (v != null) {
            ...
            } else { g.setColor(tabPane.getBackgroundAt(tabIndex).brighter());
                SwingUtilities2.drawStringUnderlineCharAt(tabPane, g,
                             title, mnemIndex,
                             textRect.x, textRect.y + metrics.getAscent());
    ...
    protected void paintTabBorder(Graphics g, int tabPlacement,
        ...
        switch (tabPlacement) {
          case LEFT:
              g.drawLine(x+1, y+h-2, x+1, y+h-2); g.drawLine(x, y+2, x, y+h-3); g.drawLine(x+1, y+1, x+1, y+1); g.drawLine(x+2, y, x+w-1, y); g.setColor(shadow);
              g.drawLine(x+2, y+h-2, x+w-1, y+h-2); g.setColor(darkShadow);
              g.drawLine(x+2, y+h-1, x+w-1, y+h-1); break;
          case RIGHT:
              g.drawLine(x, y, x+w-3, y); g.setColor(shadow);
              g.drawLine(x, y+h-2, x+w-3, y+h-2); g.drawLine(x+w-2, y+2, x+w-2, y+h-3); g.setColor(darkShadow);
              g.drawLine(x+w-2, y+1, x+w-2, y+1); g.drawLine(x+w-2, y+h-2, x+w-2, y+h-2); g.drawLine(x+w-1, y+2, x+w-1, y+h-3); g.drawLine(x, y+h-1, x+w-3, y+h-1); break;
          case BOTTOM:
              g.drawLine(x, y, x, y+h-3); g.drawLine(x+1, y+h-2, x+1, y+h-2); g.setColor(shadow);
              g.drawLine(x+2, y+h-2, x+w-3, y+h-2); g.drawLine(x+w-2, y, x+w-2, y+h-3); g.setColor(darkShadow);
              g.drawLine(x+2, y+h-1, x+w-3, y+h-1); g.drawLine(x+w-2, y+h-2, x+w-2, y+h-2); g.drawLine(x+w-1, y, x+w-1, y+h-3); break;
          case TOP:
          default:
              g.drawLine(x, y+2, x, y+h-1); g.drawLine(x+1, y+1, x+1, y+1); g.drawLine(x+2, y, x+w-3, y); g.setColor(shadow);
              g.drawLine(x+w-2, y+2, x+w-2, y+h-1); g.setColor(darkShadow);
              g.drawLine(x+w-1, y+2, x+w-1, y+h-1); g.drawLine(x+w-2, y+1, x+w-2, y+1); }
    }
...
public Rectangle getTabBounds(JTabbedPane pane, int i) {
    ...
    protected Rectangle getTabBounds(int tabIndex, Rectangle dest) {
        ...
        if (scrollableTabLayoutEnabled()) { Point vpp = tabScroller.viewport.getLocation();
            Point viewp = tabScroller.viewport.getViewPosition();
            dest.x = rects[tabIndex].x + vpp.x - viewp.x;
            dest.y = rects[tabIndex].y + vpp.y - viewp.y;

        } else { dest.x = rects[tabIndex].x;
...
protected void navigateSelectedTab(int direction) {
    ...
    public class TabbedPaneLayout implements LayoutManager {
        ...
        protected void padSelectedTab(int tabPlacement, int selectedIndex) {
            ...
            if (selectedIndex >= 0) {
                ...
                if (!scrollableTabLayoutEnabled()) { Dimension size = tabPane.getSize();
                    Insets insets = tabPane.getInsets();
    ...
    private class TabbedPaneScrollLayout extends TabbedPaneLayout {
        ...
        public void layoutContainer(Container parent) {
            ...
            int tx, ty, tw, th; int cx, cy, cw, ch; Insets contentInsets = getContentBorderInsets(tabPlacement);
            Rectangle bounds = tabPane.getBounds();
            int numChildren = tabPane.getComponentCount();
    ...
    private class ScrollableTabSupport implements ActionListener,
        ...
        void createButtons() {
            ...
            } else { scrollForwardButton = createScrollButton(SOUTH);
...
private class Handler implements ChangeListener, ContainerListener,
        ...
        private void updateHtmlViews(int index) {
            ...
            if (isHTML) {
                if (htmlViews==null) {    htmlViews = createHTMLVector();
                } else {                  View v = BasicHTML.createHTMLView(tabPane, title);
                    htmlViews.insertElementAt(v, index);
                }
            } else {                             if (htmlViews!=null) {           htmlViews.insertElementAt(null, index);
                }                                }
            updateMnemonics();
        }
