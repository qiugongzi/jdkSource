    ...
    private class Handler implements ChangeListener, MouseListener,
        ...
        public void propertyChange(PropertyChangeEvent evt) {
            ...
            if (prop == JColorChooser.CHOOSER_PANELS_PROPERTY) {
                ...
                for (int i = 0; i < oldPanels.length; i++) {  Container wrapper = oldPanels[i].getParent();
                    if (wrapper != null) {
                      Container parent = wrapper.getParent();
                      if (parent != null)
                          parent.remove(wrapper);  oldPanels[i].uninstallChooserPanel(chooser); }
                }

                int numNewPanels = newPanels.length;
                if (numNewPanels == 0) {  chooser.remove(tabbedPane);
                    return;
                }
                else if (numNewPanels == 1) {  chooser.remove(tabbedPane);
                ...
                else {   if ( oldPanels.length < 2 ) {chooser.remove(singlePanel);
