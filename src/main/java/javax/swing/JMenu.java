...
public class JMenu extends JMenuItem implements Accessible,MenuElement
{
    ...
    private static final boolean TRACE =   false; private static final boolean VERBOSE = false; private static final boolean DEBUG =   false;  public JMenu() {
        this("");
    }
    ...
    protected Point getPopupMenuOrigin() {
        ...
        if (parent instanceof JPopupMenu) {
            ...
            if( SwingUtilities.isLeftToRight(this) ) {
                x = s.width + xOffset;   if (position.x + x + pmSize.width >= screenBounds.width
                                                     + screenBounds.x &&
                ...
                x = 0 - xOffset - pmSize.width; if (position.x + x < screenBounds.x &&
                    screenBounds.width - s.width > 2*(position.x -
                                                    screenBounds.x)) {
            ...
            y = yOffset;                     if (position.y + y + pmSize.height >= screenBounds.height
                                                  + screenBounds.y &&
            ...
            if( SwingUtilities.isLeftToRight(this) ) {
                x = xOffset;                   if (position.x + x + pmSize.width >= screenBounds.width
                                                     + screenBounds.x &&
                    screenBounds.width - s.width < 2*(position.x
                                                    - screenBounds.x)) {
                ...
                x = s.width - xOffset - pmSize.width; if (position.x + x < screenBounds.x &&
                    screenBounds.width - s.width > 2*(position.x
                                                    - screenBounds.x)) {
            ...
            y = s.height + yOffset;    if (position.y + y + pmSize.height >= screenBounds.height
                                                  + screenBounds.y &&
                screenBounds.height - s.height < 2*(position.y
                                                  - screenBounds.y)) {
        ...
        public int getAccessibleSelectionCount() {
            ...
            if (me != null) {
                for (int i = 0; i < me.length; i++) {
                    if (me[i] == JMenu.this) {   if (i+1 < me.length) {
                            return 1;
                        }
                    }
                }
        ...
        public Accessible getAccessibleSelection(int i) {
            ...
            if (me != null) {
                for (int j = 0; j < me.length; j++) {
                    if (me[j] == JMenu.this) {   while (++j < me.length) {
        ...
        public void clearAccessibleSelection() {
            ...
            if (old != null) {
                for (int j = 0; j < old.length; j++) {
                    if (old[j] == JMenu.this) {  MenuElement me[] = new MenuElement[j+1];
