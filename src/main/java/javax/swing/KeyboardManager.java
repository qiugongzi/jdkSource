...
class KeyboardManager {
    ...
    public static void setCurrentManager(KeyboardManager km) {
     ...
     public void registerKeyStroke(KeyStroke k, JComponent c) {
         ...
         if (keyMap ==  null) {  keyMap = registerNewTopContainer(topContainer);
         }

         Object tmp = keyMap.get(k);
         if (tmp == null) {
             keyMap.put(k,c);
         } else if (tmp instanceof Vector) {  Vector v = (Vector)tmp;
             if (!v.contains(c)) {  v.addElement(c);
             }
         } else if (tmp instanceof JComponent) {
           if (tmp != c) {  Vector<JComponent> v = new Vector<JComponent>();
     ...
     public void unregisterKeyStroke(KeyStroke ks, JComponent c) {
       ...
       ComponentKeyStrokePair ckp = new ComponentKeyStrokePair(c,ks);
         ...
         Hashtable keyMap = containerMap.get(topContainer);
         if  (keyMap == null) { Thread.dumpStack();
             return;
         }

         Object tmp = keyMap.get(ks);
         if (tmp == null) {  Thread.dumpStack();
             return;
         }

         if (tmp instanceof JComponent && tmp == c) {
             keyMap.remove(ks);  } else if (tmp instanceof Vector ) {  Vector v = (Vector)tmp;
             v.removeElement(c);
             if ( v.isEmpty() ) {
                 keyMap.remove(ks);  }
         }

         if ( keyMap.isEmpty() ) {  containerMap.remove(topContainer);  }
    ...
    public boolean fireKeyboardAction(KeyEvent e, boolean pressed, Container topAncestor) {
         ...
         if (keyMap != null) { Object tmp = null;
             if( ksE != null ) {
                 tmp = keyMap.get(ksE);
                 if( tmp != null ) {
                     ks = ksE;
             ...
             if (tmp == null) {
                 ...
                 if ( c.isShowing() && c.isEnabled() ) { fireBinding(c, ks, e, pressed);
                 }
             } else if ( tmp instanceof Vector) { Vector v = (Vector)tmp;
         ...
         if ( keyMap != null) {
             ...
             if (v != null) {
                 ...
                 while (iter.hasMoreElements()) {
                     ...
                     if ( mb.isShowing() && mb.isEnabled() ) { boolean extended = (ksE != null) && !ksE.equals(ks);
                         if (extended) {
    ...
    public void registerMenuBar(JMenuBar mb) {
        ...
        if (keyMap ==  null) {  keyMap = registerNewTopContainer(top);
        }
        Vector menuBars = (Vector)keyMap.get(JMenuBar.class);

        if (menuBars == null) {  menuBars = new Vector();
