...
public class MetaMessage extends MidiMessage {
    ...
    public static final int META                                                = 0xFF; private int dataLength = 0;
    ...
    public void setMessage(int type, byte[] data, int length) throws InvalidMidiDataException {
        ...
        this.data[0] = (byte) META;        this.data[1] = (byte) type;        writeVarInt(this.data, 2, length); if (length > 0) {
    ...
    private void writeVarInt(byte[] data, int off, long value) {
        int shift=63; while ((shift > 0) && ((value & (mask << shift)) == 0)) shift-=7;
        while (shift > 0) {
