...
public abstract class SetOfIntegerSyntax implements Serializable, Cloneable {
    ...
    private static int[][] parse(String members) {
        ...
        while (i < n) {
            ...
            case 0: if (Character.isWhitespace(c)) {
                    state = 0;
                }
            ...
            case 1: if (Character.isWhitespace(c)){
                        state = 2;
                } else if ((digit = Character.digit(c, 10)) != -1) {
                    lb = 10 * lb + digit;
                    state = 1;
            ...
            case 2: if (Character.isWhitespace(c)) {
                    state = 2;
                }
                else if (c == '-' || c == ':') {
                    state = 3;
            ...
            case 3: if (Character.isWhitespace(c)) {
                    state = 3;
                } else if ((digit = Character.digit(c, 10)) != -1) {
                    ub = digit;
                    state = 4;
            ...
            case 4: if (Character.isWhitespace(c)) {
                    state = 5;
                } else if ((digit = Character.digit(c, 10)) != -1) {
                    ub = 10 * ub + digit;
                    state = 4;
            ...
            case 5: if (Character.isWhitespace(c)) {
                    state = 5;
                } else if (c == ',') {
                    accumulate(theRanges, lb, ub);
                    state = 6;
            ...
            case 6: if (Character.isWhitespace(c)) {
                    state = 6;
                } else if ((digit = Character.digit(c, 10)) != -1) {
                    lb = digit;
                    state = 1;
        ...
        switch (state) {
        case 0: break;
        case 1: case 2: accumulate(theRanges, lb, lb);
            break;
        case 4: case 5: accumulate(theRanges, lb, ub);
            break;
        case 3: case 6: throw new IllegalArgumentException();
        }
