...
class LambdaForm {
    ...
    MemberName vmentry;   private boolean isCompiled;
    ...
    enum BasicType {
        L_TYPE('L', Object.class, Wrapper.OBJECT),  I_TYPE('I', int.class,    Wrapper.INT),
        J_TYPE('J', long.class,   Wrapper.LONG),
        F_TYPE('F', float.class,  Wrapper.FLOAT),
        D_TYPE('D', double.class, Wrapper.DOUBLE),  V_TYPE('V', void.class,   Wrapper.VOID);    static final BasicType[] ALL_TYPES = BasicType.values();
    ...
    private static Name[] buildEmptyNames(int arity, String basicTypeSignature) {
        ...
        int resultPos = arity + 1;  if (arity < 0 || basicTypeSignature.length() != resultPos+1)
    ...
    private static int fixResult(int result, Name[] names) {
        if (result == LAST_RESULT)
            result = names.length - 1;  if (result >= 0 && names[result].type == V_TYPE)
            result = VOID_RESULT;
        return result;
    }
    ...
    private static String fixDebugName(String debugName) {
        if (DEBUG_NAME_COUNTERS != null) {
            ...
            if (under < length) {
                ++under;    while (under < length && Character.isDigit(debugName.charAt(under))) {
                    ++under;
                }
    ...
    LambdaForm customize(MethodHandle mh) {
        ...
        customForm.transformCache = this; return customForm;
    }
    ...
    LambdaForm uncustomize() {
        ...
        assert(transformCache != null); LambdaForm uncustomizedForm = (LambdaForm)transformCache;
        if (COMPILE_THRESHOLD > 0 && isCompiled) {
    ...
    static boolean isValidSignature(String sig) {
        ...
        if (arity < 0)  return false;  int siglen = sig.length();
        if (siglen != arity + 2)  return false;  for (int i = 0; i < siglen; i++) {
            if (i == arity)  continue;  char c = sig.charAt(i);
            if (c == 'V')
    ...
    private static boolean valueMatches(BasicType tc, Class<?> type, Object x) {
        if (type == void.class)  tc = V_TYPE;   assert tc == basicType(type) : tc + " == basicType(" + type + ")=" + basicType(type);
        switch (tc) {
        ...
        case V_TYPE: break;  default:  assert(false);
        }
        return true;
    }
    ...
    private void checkInvocationCounter() {
        if (COMPILE_THRESHOLD != 0 &&
            ...
            invocationCounter++;  if (invocationCounter >= COMPILE_THRESHOLD) {
                compileToBytecode();
            }
        }
    }
    Object interpretWithArgumentsTracing(Object... argumentValues) throws Throwable {
        ...
        if (invocationCounter < COMPILE_THRESHOLD) {
            int ctr = invocationCounter++;  traceInterpreter("| invocationCounter", ctr);
            if (invocationCounter >= COMPILE_THRESHOLD) {
                compileToBytecode();
            }
        }
    ...
    static class NamedFunction {
        ...
        private static MethodHandle computeInvoker(MethodTypeForm typeForm) {
            typeForm = typeForm.basicType().form();  MethodHandle mh = typeForm.cachedMethodHandle(MethodTypeForm.MH_NF_INV);
            if (mh != null)  return mh;
            MemberName invoker = InvokerBytecodeGenerator.generateNamedFunctionInvoker(typeForm);  mh = DirectMethodHandle.make(invoker);
            MethodHandle mh2 = typeForm.cachedMethodHandle(MethodTypeForm.MH_NF_INV);
            if (mh2 != null)  return mh2;  if (!mh.type().equals(INVOKER_METHOD_TYPE))
        ...
        private static boolean checkArgumentTypes(Object[] arguments, MethodType methodType) {
            if (true)  return true;  MethodType dstType = methodType.form().erasedType();
    ...
    static final class Name {
        ...
        final Object constraint;  @Stable final Object[] arguments;
        ...
        private Name(Name that, Object constraint) {
            ...
            assert(constraint == null || isParam());  assert(constraint == null || constraint instanceof BoundMethodHandle.SpeciesData || constraint instanceof Class);
        }
        ...
        Name replaceName(Name oldName, Name newName) {  if (oldName == newName)  return this;
    ...
    int lastUseIndex(Name n) {
        ...
        assert(names[ni] == n);
        if (result == ni)  return nmax;  for (int i = nmax; --i > ni; ) {
    ...
    private static void identity_V() { return; }  private static int zero_I() { return 0; }
