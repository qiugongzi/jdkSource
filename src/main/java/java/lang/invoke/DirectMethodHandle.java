...
class DirectMethodHandle extends MethodHandle {
    ...
    private DirectMethodHandle(MethodType mtype, LambdaForm form, MemberName member) {
        ...
        if (member.getDeclaringClass().isInterface() &&
            ...
            if (m != null && m.isPublic()) {
                assert(member.getReferenceKind() == m.getReferenceKind());  member = m;
            }
        }
    ...
    MethodHandle copyWith(MethodType mt, LambdaForm lf) {
        assert(this.getClass() == DirectMethodHandle.class);  return new DirectMethodHandle(mt, lf, member);
    }
    ...
    private static LambdaForm preparedLambdaForm(MemberName m) {
        assert(m.isInvocable()) : m;  MethodType mtype = m.getInvocationType().basicType();
    ...
    private static LambdaForm makePreparedLambdaForm(MethodType mtype, int which) {
        ...
        if (doesAlloc)
            mtypeWithArg = mtypeWithArg
                    .insertParameterTypes(0, Object.class)  .changeReturnType(void.class);          MemberName linker = new MemberName(MethodHandle.class, linkerName, mtypeWithArg, REF_invokeStatic);
        try {
        ...
        assert(outArgs[outArgs.length-1] == names[GET_MEMBER]);  int result = LAST_RESULT;
        if (doesAlloc) {
            assert(outArgs[outArgs.length-2] == names[NEW_OBJ]);  System.arraycopy(outArgs, 0, outArgs, 1, outArgs.length-2);
    ...
    private static LambdaForm preparedFieldLambdaForm(byte formOp, boolean isVolatile, Class<?> ftype) {
        ...
        ACCESSOR_FORMS[afIndex] = lform;  return lform;
    }
    ...
    private static LambdaForm makePreparedFieldLambdaForm(byte formOp, boolean isVolatile, int ftypeKind) {
        ...
        String linkerName = (getOrPut + ctname);  MethodType linkerType;
        if (isGetter)
        ...
        mtype = mtype.basicType();  if (!isStatic)
        ...
        int nameCursor = ARG_LIMIT;
        final int F_HOLDER  = (isStatic ? nameCursor++ : -1);  final int F_OFFSET  = nameCursor++;  final int OBJ_CHECK = (OBJ_BASE >= 0 ? nameCursor++ : -1);
        final int INIT_BAR  = (needsInit ? nameCursor++ : -1);
        final int PRE_CAST  = (needsCast && !isGetter ? nameCursor++ : -1);
        final int LINKER_CALL = nameCursor++;
        final int POST_CAST = (needsCast && isGetter ? nameCursor++ : -1);
        final int RESULT    = nameCursor-1;  Name[] names = arguments(nameCursor - ARG_LIMIT, mtype.invokerType());
        if (needsInit)
        ...
        String lambdaName = (linkerName + fieldOrStatic);  if (needsCast)  lambdaName += "Cast";
