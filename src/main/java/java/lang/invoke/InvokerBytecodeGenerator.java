...
class InvokerBytecodeGenerator {
    ...
    private final int[]       localsMap;    private final BasicType[] localTypes;   private final Class<?>[]  localClasses; private ClassWriter cw;
    ...
    int cph = 0;  String constantPlaceholder(Object arg) {
        String cpPlaceholder = "CONSTANT_PLACEHOLDER_" + cph++;
        if (DUMP_CLASS_FILES) cpPlaceholder += " <<" + debugString(arg) + ">>";  if (cpPatches.containsKey(cpPlaceholder)) {
            throw new InternalError("observed CP placeholder twice: " + cpPlaceholder);
        }
        int index = cw.newConst((Object) cpPlaceholder);  cpPatches.put(cpPlaceholder, new CpPatch(index, cpPlaceholder, arg));
    ...
    private static Class<?> loadAndInitializeInvokerClass(byte[] classBytes, Object[] patches) {
        ...
        UNSAFE.ensureClassInitialized(invokerClass);  return invokerClass;
    }
    ...
    private void classFilePrologue() {
        final int NOT_ACC_PUBLIC = 0;  cw = new ClassWriter(ClassWriter.COMPUTE_MAXS + ClassWriter.COMPUTE_FRAMES);
    ...
    private byte arrayTypeCode(Wrapper elementType) {
        switch (elementType) {
            ...
            case OBJECT:  return 0; default:      throw new InternalError();
        }
    }
    ...
    private void emitImplicitConversion(BasicType ptype, Class<?> pclass, Object arg) {
        assert(basicType(pclass) == ptype);  if (pclass == ptype.basicTypeClass() && ptype != L_TYPE)
            return;   switch (ptype) {
            case L_TYPE:
    ...
    private boolean assertStaticType(Class<?> cls, Name n) {
        ...
        if (aclass != null && (aclass == cls || cls.isAssignableFrom(aclass))) {
            return true;  } else if (aclass == null || aclass.isAssignableFrom(cls)) {
            localClasses[local] = cls;  }
        return false;
    }

    private void emitReferenceCast(Class<?> cls, Object arg) {
        Name writeBack = null;  if (arg instanceof Name) {
            Name n = (Name) arg;
            if (assertStaticType(cls, n))
                return;  if (lambdaForm.useCount(n) > 1) {
                writeBack = n;
            }
        }
    ...
    private byte[] generateCustomizedCodeBytes() {
        ...
        if (lambdaForm.customized != null) {
            ...
            assert(checkActualReceiver()); mv.visitVarInsn(Opcodes.ASTORE, localsMap[0]);
        }
        ...
        for (int i = lambdaForm.arity; i < lambdaForm.names.length; i++) {
            ...
            onStack = name;  MethodHandleImpl.Intrinsic intr = name.function.intrinsicName();
            switch (intr) {
    ...
    void emitInvoke(Name name) {
        assert(!isLinkerMethodInvoke(name));  if (true) {
    ...
    static boolean isStaticallyInvocable(MemberName member) {
        ...
        if (cls.isArray() || cls.isPrimitive())
            return false;  if (cls.isAnonymousClass() || cls.isLocalClass())
            return false;  if (cls.getClassLoader() != MethodHandle.class.getClassLoader())
            return false;  if (ReflectUtil.isVMAnonymousClass(cls)) return false;
        ...
        if (!member.isPrivate() && VerifyAccess.isSamePackage(MethodHandle.class, cls))
            return true;   if (member.isPublic() && isStaticallyNameable(cls))
            return true;
        return false;
    }
    ...
    static boolean isStaticallyNameable(Class<?> cls) {
        ...
        if (cls.isPrimitive())
            return true;  if (ReflectUtil.isVMAnonymousClass(cls)) return false;
    ...
    void emitNewArray(Name name) throws InternalError {
        ...
        if (name.arguments.length == 0) {
            ...
            assert(emptyArray.getClass() == rtype);  mv.visitLdcInsn(constantPlaceholder(emptyArray));
    ...
    private boolean isInvokeBasic(Name name) {
        ...
        if (name.arguments.length < 1)
            return false;  MemberName member = name.function.member();
    ...
    private boolean isSelectAlternative(int pos) {
        ...
        return nameRefersTo(name0, MethodHandleImpl.class, "selectAlternative") &&
               isInvokeBasic(name1) &&
               name1.lastUseIndex(name0) == 0 &&        lambdaForm.lastUseIndex(name0) == pos+1; }
    ...
    private boolean isGuardWithCatch(int pos) {
        ...
        return nameRefersTo(name1, MethodHandleImpl.class, "guardWithCatch") &&
               isInvokeBasic(name0) &&
               isInvokeBasic(name2) &&
               name1.lastUseIndex(name0) == 3 &&          lambdaForm.lastUseIndex(name0) == pos+1 && name2.lastUseIndex(name1) == 1 &&          lambdaForm.lastUseIndex(name1) == pos+2;   }
    ...
    private Name emitSelectAlternative(Name selectAlternativeName, Name invokeBasicName) {
        ...
        emitPushArgument(selectAlternativeName, 1);  emitAstoreInsn(receiver.index());  emitStaticInvoke(invokeBasicName);
        ...
        emitPushArgument(selectAlternativeName, 2);  emitAstoreInsn(receiver.index());  emitStaticInvoke(invokeBasicName);
    ...
    private Name emitGuardWithCatch(int pos) {
        ...
        emitPushArguments(args, 1); mv.visitMethodInsn(Opcodes.INVOKEVIRTUAL, MH, "invokeBasic", type.basicType().toMethodDescriptorString(), false);
        ...
        mv.visitInsn(Opcodes.SWAP);
        emitPushArguments(args, 1); MethodType catcherType = type.insertParameterTypes(0, Throwable.class);
    ...
    private void emitReturn(Name onStack) {
        ...
        assert(rtype == basicType(rclass));  if (rtype == V_TYPE) {
    ...
    static MemberName generateLambdaFormInterpreterEntryPoint(String sig) {
        ...
        MethodType type = signatureType(sig);  type = type.changeParameterType(0, MethodHandle.class);
    ...
    private byte[] generateLambdaFormInterpreterEntryPointBytes() {
        ...
        mv.visitInsn(Opcodes.SWAP);  mv.visitMethodInsn(Opcodes.INVOKEVIRTUAL, LF, "interpretWithArguments", "([Ljava/lang/Object;)Ljava/lang/Object;", false);
    ...
    private byte[] generateNamedFunctionInvokerImpl(MethodTypeForm typeForm) {
        ...
        for (int i = 0; i < dstType.parameterCount(); i++) {
            ...
            if (dptype.isPrimitive()) {
                ...
                Wrapper srcWrapper = dstWrapper.isSubwordOrInt() ? Wrapper.INT : dstWrapper;  emitUnboxing(srcWrapper);
        ...
        if (rtype != void.class && rtype.isPrimitive()) {
            ...
            Wrapper dstWrapper = srcWrapper.isSubwordOrInt() ? Wrapper.INT : srcWrapper;  emitPrimCast(srcWrapper, dstWrapper);
        ...
        emitReturnInsn(L_TYPE);  classFileEpilogue();
        bogusMethod(dstType);
