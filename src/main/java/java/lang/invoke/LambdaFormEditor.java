...
class LambdaFormEditor {
    ...
    private static final class Transform extends SoftReference<LambdaForm> {
        ...
        private enum Kind {
            NO_KIND,  BIND_ARG, ADD_ARG, DUP_ARG,
            SPREAD_ARGS,
        ...
        private static final boolean STRESS_TEST = false; private static final int
        ...
        private static boolean inRange(int bitset) {
            assert((bitset & 0xFF) == bitset);  return ((bitset & ~PACKED_BYTE_MASK) == 0);
        }
        ...
        for (int pass = 0; ; pass++) {
            ...
            synchronized (lambdaForm) {
                ...
                if (c instanceof Transform) {
                    ...
                    } else if (k.get() == null) { lambdaForm.transformCache = key;
                        return form;
                    }
    ...
    LambdaForm bindArgumentForm(int pos) {
        ...
        Name oldBaseAddress = lambdaForm.parameter(0);  Name newBaseAddress;
    ...
    LambdaForm spreadArgumentsForm(int pos, Class<?> arrayType, int arrayLength) {
        ...
        assert(pos > 0);  Name spreadParam = new Name(L_TYPE);
    ...
    LambdaForm collectArgumentsForm(int pos, MethodType collectorType) {
        ...
        if (dropResult && collectorArity == 0)  pos = 1;  Transform key = Transform.of(kind, pos, collectorArity, BasicType.basicTypesOrd(newTypes));
        LambdaForm form = getInCache(key);
        if (form != null) {
    ...
    LambdaForm collectArgumentArrayForm(int pos, MethodHandle arrayCollector) {
        ...
        assert(pos > 0);  Name[] newParams = new Name[collectorArity];
        ...
        int argPos = pos + 1;  for (Name newParam : newParams) {
    ...
    private LambdaForm makeArgumentCombinationForm(int pos,
        ...
        assert(pos > 0);  assert(combinerType == combinerType.basicType());
        assert(combinerType.returnType() != void.class || dropResult);

        BoundMethodHandle.SpeciesData oldData = oldSpeciesData();
        BoundMethodHandle.SpeciesData newData = newSpeciesData(L_TYPE);

        Name oldBaseAddress = lambdaForm.parameter(0);  buf.replaceFunctions(oldData.getterFunctions(), newData.getterFunctions(), oldBaseAddress);
        ...
        int argPos = pos + resultArity;  for (Name newParam : newParams) {
            buf.insertParameter(argPos++, newParam);
        }
    ...
    LambdaForm permuteArgumentsForm(int skip, int[] reorder) {
        assert(skip == 1);  int length = lambdaForm.names.length;
        ...
        if (nullPerm)  return lambdaForm;  Transform key = Transform.of(Transform.Kind.PERMUTE_ARGS, reorder);
        LambdaForm form = getInCache(key);
        if (form != null) {
