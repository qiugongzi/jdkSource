...
final class LambdaFormBuffer {
    ...
    private Name[] names;
    private Name[] originalNames;  private byte flags;
    ...
    private LambdaForm lambdaForm() {
        assert(!inTrans());  return new LambdaForm(debugName, arity, nameArray(), resultIndex());
    }
    ...
    void setNames(Name[] names2) {
        names = originalNames = names2;  length = names2.length;
        flags = 0;
    }
    ...
    private void noteDuplicate(int pos1, int pos2) {
        Name n = names[pos1];
        assert(n == names[pos2]);
        assert(originalNames[pos1] != null);  assert(originalNames[pos2] == null || originalNames[pos2] == n);
        if (dups == null) {
            dups = new ArrayList<>();
        }
        dups.add(n);
    ...
    private void changeName(int i, Name name) {
        ...
        assert(oldName == originalNames[i]);  assert(verifyFirstChange());
        if (ownedCount() == 0)
            growNames(0, 0);
        names[i] = name;
        if (firstChange > i) {
    ...
    LambdaForm endEdit() {
        ...
        for (int i = Math.max(firstChange, arity); i < length; i++) {
            Name name = names[i];
            if (name == null)  continue;  Name newName = name.replaceNames(originalNames, names, firstChange, i);
            if (newName != name) {
                names[i] = newName;
                if (resultName == name) {
                    resultName = newName;
    ...
    LambdaFormBuffer replaceParameterByNewExpression(int pos, Name binding) {
        ...
        assert(lastIndexOf(binding) < 0);  replaceName(pos, binding);
        return this;
    }
    ...
    LambdaFormBuffer replaceParameterByCopy(int pos, int valuePos) {
        ...
        noteDuplicate(pos, valuePos);  return this;
    }
