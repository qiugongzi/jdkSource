...
abstract class DelegatingMethodHandle extends MethodHandle {
    ...
    private static LambdaForm chooseDelegatingForm(MethodHandle target) {
        if (target instanceof SimpleMethodHandle)
            return target.internalForm();  return makeReinvokerForm(target, MethodTypeForm.LF_DELEGATE, DelegatingMethodHandle.class, NF_getTarget);
    }
    ...
    static LambdaForm makeReinvokerForm(MethodHandle target,
        ...
        if (customized) {
            ...
            names[REINVOKE] = new LambdaForm.Name(target, targetArgs);  } else {
            ...
            targetArgs[0] = names[NEXT_MH];  names[REINVOKE] = new LambdaForm.Name(mtype, targetArgs);
        }
