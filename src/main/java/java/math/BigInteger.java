...
public class BigInteger extends Number implements Comparable<BigInteger> {
    ...
    private static final int MAX_MAG_LENGTH = Integer.MAX_VALUE / Integer.SIZE + 1; private static final  int PRIME_SEARCH_BIT_LENGTH_LIMIT = 500000000;
    ...
    private static byte[] randomBits(int numBits, Random rnd) {
        ...
        int numBytes = (int)(((long)numBits+7)/8); byte[] randomBits = new byte[numBytes];
    ...
    private static BigInteger smallPrime(int bitLength, int certainty, Random rnd) {
        ...
        int highBit = 1 << ((bitLength+31) & 0x1f);  int highMask = (highBit << 1) - 1;  while (true) {
            for (int i=0; i < magLen; i++)
                temp[i] = rnd.nextInt();
            temp[0] = (temp[0] & highMask) | highBit;  if (bitLength > 2)
                temp[magLen-1] |= 1;  BigInteger p = new BigInteger(temp, 1);
    ...
    private static int[] subtract(long val, int[] little) {
        ...
        if (highWord == 0) {
            ...
            if (little.length == 1) {
                ...
                return result;
            } else { long difference = ((int)val & LONG_MASK) - (little[1] & LONG_MASK);
                result[1] = (int)difference;
    ...
    BigInteger multiply(long v) {
        ...
        if (v < 0)
            v = -v;
        long dh = v >>> 32;      long dl = v & LONG_MASK; int xlen = mag.length;
        int[] value = mag;
    ...
    private static BigInteger multiplyKaratsuba(BigInteger x, BigInteger y) {
        ...
        BigInteger p1 = xh.multiply(yh);  BigInteger p2 = xl.multiply(yl);  BigInteger p3 = xh.add(xl).multiply(yh.add(yl));
    ...
    private static BigInteger multiplyToomCook3(BigInteger a, BigInteger b) {
        ...
        int k = (largest+2)/3;   int r = largest - 2*k;
    ...
    private BigInteger squareKaratsuba() {
        ...
        BigInteger xhs = xh.square();  BigInteger xls = xl.square();  return xhs.shiftLeft(half*32).add(xl.add(xh).square().subtract(xhs.add(xls))).shiftLeft(half*32).add(xls);
    }
    ...
    public BigInteger pow(int exponent) {
        ...
        if (powersOfTwo > 0) {
            ...
            if (remainingBits == 1) {  if (signum < 0 && (exponent&1) == 1) {
        ...
        if (partToSquare.mag.length == 1 && scaleFactor <= 62) {
            ...
            if (powersOfTwo > 0) {
                if (bitsToShift + scaleFactor <= 62) { return valueOf((result << bitsToShift) * newSign);
                } else {
    ...
    public BigInteger modPow(BigInteger exponent, BigInteger m) {
        ...
        BigInteger base = (this.signum < 0 || this.compareTo(m) >= 0
                           ? this.mod(m) : this);
        BigInteger result;
        if (m.testBit(0)) { result = base.oddModPow(exponent, m);
        } else {


            int p = m.getLowestSetBit();   BigInteger m1 = m.shiftRight(p);  BigInteger m2 = ONE.shiftLeft(p); BigInteger base2 = (this.signum < 0 || this.compareTo(m1) >= 0
                                ? this.mod(m1) : this);
    ...
    static int[] bnExpModThreshTable = {7, 25, 81, 241, 673, 1793,
                                                Integer.MAX_VALUE}; private BigInteger oddModPow(BigInteger y, BigInteger z) {
        ...
        b2.normalize(); MutableBigInteger r= a2.divide(b2, q);
    ...
    public int bitCount() {
        ...
        if (bc == -1) {  bc = 0;      for (int i=0; i < mag.length; i++)
    ...
    private static BigInteger getRadixConversionCache(int radix, int exponent) {
        BigInteger[] cacheLine = powerCache[radix]; if (exponent < cacheLine.length) {
        ...
        BigInteger[][] pc = powerCache; if (exponent >= pc[radix].length) {
            pc = pc.clone();
            pc[radix] = cacheLine;
            powerCache = pc; }
        return cacheLine[exponent];
    ...
    public float floatValue() {
        ...
        signifFloor &= FloatConsts.SIGNIF_BIT_MASK; boolean increment = (twiceSignifFloor & 1) != 0
    ...
    public double doubleValue() {
        ...
        signifFloor &= DoubleConsts.SIGNIF_BIT_MASK; boolean increment = (twiceSignifFloor & 1) != 0
                && ((signifFloor & 1) != 0 || abs().getLowestSetBit() < shift);
