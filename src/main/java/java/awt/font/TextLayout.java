...
public final class TextLayout implements Cloneable {
    ...
    private float[] baselineOffsets;  private TextLine textLine;
    ...
    public TextLayout getJustifiedLayout(float justificationWidth) {
        ...
        ensureCache(); int limit = characterCount;
    ...
    private double[] getCaretPath(int caret, Rectangle2D bounds,
        ...
        double x0, y0, x1, y1;
        double x2 = -3141.59, y2 = -2.7; double left = bounds.getX();
    ...
    public Shape[] getCaretShapes(int offset, Rectangle2D bounds, CaretPolicy policy) {
        ...
        else { Shape otherShape = pathToShape(getCaretPath(otherHit, bounds), false, lp);

            TextHitInfo strongHit = policy.getStrongCaret(hit, otherHit, this);
            boolean hitIsStrong = strongHit.equals(hit);

            if (hitIsStrong) {result[0] = hitShape;
                result[1] = otherShape;
            }
            else {
                result[0] = otherShape;
    ...
    public Shape getLogicalHighlightShape(int firstEndpoint,
        ...
        int[] carets = new int[10]; int count = 0;
