...
public class ComponentSampleModel extends SampleModel
{
    ...
    public SampleModel createCompatibleSampleModel(int w, int h) {
        ...
        if (pStride > lStride) {
            if (pStride > bStride) {
                if (lStride > bStride) { bandOff = new int[bandOffsets.length];
                    for (int i=0; i<bands; i++)
                        bandOff[i] = bandOffsets[i]-minBandOff;
                    lStride = bStride+1;
                    pStride = lStride*h;
                } else { bandOff = orderBands(bandOffsets,lStride*h);
                    pStride = bands*lStride*h;
                }
            } else { pStride = lStride*h;
                bandOff = orderBands(bandOffsets,pStride*w);
            }
        } else {
            if (pStride > bStride) { bandOff = new int[bandOffsets.length];
                for (int i=0; i<bands; i++)
                    bandOff[i] = bandOffsets[i]-minBandOff;
                pStride = bStride+1;
                lStride = pStride*w;
            } else {
                if (lStride > bStride) { bandOff = orderBands(bandOffsets,pStride*w);
                    lStride = bands*pStride*w;
                } else { lStride = pStride*w;
