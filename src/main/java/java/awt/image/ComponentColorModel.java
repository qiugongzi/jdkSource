...
public class ComponentColorModel extends ColorModel {
    ...
    private boolean signed; private boolean is_sRGB_stdScale;
    ...
    private void initScale() {
        needScaleInit = false; if (nonStdScale || signed) {
            noUnnorm = true;
        } else {
            noUnnorm = false;
        }
    ...
    public int getRGB(Object inData) {
        ...
        if (is_sRGB_stdScale || is_LinearRGB_stdScale) {
            ...
            int gray = getRed(inData); return (getAlpha(inData) << 24)
                | (gray << 16)
                | (gray <<  8)
                | gray;
        }
