...
final class LinearGradientPaintContext extends MultipleGradientPaintContext {
                               ...
                               ColorSpaceType colorSpace)
    {
        ...
        float dx = endx - startx;  float dy = endy - starty;  float dSq = dx*dx + dy*dy; float constX = dx/dSq;
        float constY = dy/dSq;
                              ...
                              int x, int y, int w, int h)
    {
        ...
        for (int i = 0; i < h; i++) { g = initConst + dgdY*(y+i);

            while (off < rowLimit) { pixels[off++] = indexIntoGradientsArrays(g);
