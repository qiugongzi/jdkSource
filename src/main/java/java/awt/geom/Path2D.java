...
public abstract class Path2D implements Shape, Cloneable {
    ...
    static final int EXPAND_MIN = 10; Path2D() {
    }
    ...
    static byte[] expandPointTypes(byte[] oldPointTypes, int needed) {
        ...
        int grow = oldSize;
        if (grow > EXPAND_MAX) {
            grow = Math.max(EXPAND_MAX, oldSize >> 3); } else if (grow < EXPAND_MIN) {
            grow = EXPAND_MIN;
        }
        assert grow > 0;
    ...
    public static class Float extends Path2D implements Serializable {
        ...
        static float[] expandCoords(float[] oldCoords, int needed) {
            ...
            if (grow > EXPAND_MAX_COORDS) {
                grow = Math.max(EXPAND_MAX_COORDS, oldSize >> 3); } else if (grow < EXPAND_MIN) {
                grow = EXPAND_MIN;
            }
            assert grow > needed;
