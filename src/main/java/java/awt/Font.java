...
public class Font implements java.io.Serializable
{
    ...
    private transient long pData;       private transient Font2DHandle font2DHandle;
    ...
    private AttributeValues getAttributeValues() {
        if (values == null) {
            ...
            valuesTmp.setSize(pointSize); if ((style & BOLD) != 0) {
                valuesTmp.setWeight(2); }

            if ((style & ITALIC) != 0) {
                valuesTmp.setPosture(.2f); }
            valuesTmp.defineAll(PRIMARY_MASK); values = valuesTmp;
        }
    ...
    private void initFromValues(AttributeValues values) {
        this.values = values;
        values.defineAll(PRIMARY_MASK); this.name = values.getFamily();
        this.pointSize = values.getSize();
        this.size = (int)(values.getSize() + 0.5);
        if (values.getWeight() >= 2f) this.style |= BOLD; if (values.getPosture() >= .2f) this.style |= ITALIC; this.nonIdentityTx = values.anyNonDefault(EXTRA_MASK);
    ...
    public static Font getFont(Map<? extends Attribute, ?> attributes) {
        ...
        if (font != null) {
            if (attributes.size() > 1) { AttributeValues values = font.getAttributeValues().clone();
                values.merge(attributes, SECONDARY_MASK);
                return new Font(values, font.name, font.style,
                                font.createdFont, font.font2DHandle);
            }
             ...
             java.io.IOException
    {
        ...
        if (fRequestedAttributes != null) {
            values = getAttributeValues(); AttributeValues extras =
    ...
    private FontLineMetrics defaultLineMetrics(FontRenderContext frc) {
        ...
        if (flmref == null
            ...
            int baselineIndex = 0; float[] baselineOffsets = { 0, (descent/2f - ascent) / 2f, -ascent };
            ...
            if (isTransformed()) {
                AffineTransform ctx = values.getCharTransform(); if (ctx != null) {
    ...
    public GlyphVector layoutGlyphVector(FontRenderContext frc,
        ...
        GlyphLayout gl = GlyphLayout.get(null); StandardGlyphVector gv = gl.layout(this, frc, text,
