...
public class RuleBasedCollator extends Collator{
    ...
    public synchronized int compare(String source, String target)
    {
        ...
        while(true) {
            ...
            if ( pSOrder != pTOrder )
            {
                ...
                if (pSOrder == 0)  {
                    if (checkSecTer) {
                        result = Collator.GREATER;  checkSecTer = false;
                    }
                    gett = false;
                }
                else if (pTOrder == 0)
                {
                    if (checkSecTer) {
                        result = Collator.LESS;  checkSecTer = false;
                    }
                    gets = false;
                } else {
                    if (pSOrder < pTOrder) {
                        return Collator.LESS;  } else {
            ...
            do {
                if (CollationElementIterator.primaryOrder(tOrder) != 0)
                    return Collator.LESS; else if (CollationElementIterator.secondaryOrder(tOrder) != 0) {
                    if (checkSecTer) {
                        result = Collator.LESS;  checkSecTer = false;
                    }
                }
    ...
    public boolean equals(Object obj) {
        ...
        if (!super.equals(obj)) return false;  RuleBasedCollator other = (RuleBasedCollator) obj;
    ...
    final static int CHARINDEX = 0x70000000;  final static int EXPANDCHARINDEX = 0x7E000000; final static int CONTRACTCHARINDEX = 0x7F000000;  final static int UNMAPPED = 0xFFFFFFFF;
