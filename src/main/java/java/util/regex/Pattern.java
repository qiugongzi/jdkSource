    ...
    implements java.io.Serializable
{
    ...
    public String[] split(CharSequence input, int limit) {
        ...
        while(m.find()) {
            if (!matchLimited || matchList.size() < limit - 1) {
                ...
                matchList.add(match);
                index = m.end();
            } else if (matchList.size() == limit - 1) { String match = input.subSequence(index,
    ...
    private void compile() {
        ...
        patternLength = count;   if (! has(LITERAL))
            RemoveQEQuoting();
    ...
    private Node expr(Node end) {
        ...
        for (;;) {
            ...
            Node nodeTail = root;      if (prev == null) {
                prev = node;
                firstTail = nodeTail;
            } else {
                if (branchConn == null) {
    ...
    private Node atom() {
        ...
        for (;;) {
            ...
            case '\\':
                ch = nextEscaped();
                if (ch == 'p' || ch == 'P') { if (first > 0) { unread();
                        break;
                    } else { boolean comp = (ch == 'P');
                        boolean oneLetter = true;
                        ch = next(); if (ch != '{')
                            unread();
                        else
    ...
    private CharProperty bitsOrSingle(BitClass bits, int ch) {
        ...
        if (ch < 256 &&
            !(has(CASE_INSENSITIVE) && has(UNICODE_CASE) &&
              (ch == 0xff || ch == 0xb5 ||
               ch == 0x49 || ch == 0x69 ||  ch == 0x53 || ch == 0x73 ||  ch == 0x4b || ch == 0x6b ||  ch == 0xc5 || ch == 0xe5)))  return bits.add(ch, flags());
        return newSingle(ch);
    }
        ...
        if (ch == '\\') {
            ...
            if (ch == 'p' || ch == 'P') { boolean comp = (ch == 'P');
                ...
                if (ch != '{')
                    ...
                    oneLetter = false;
                return family(oneLetter, comp);
            } else { boolean isrange = temp[cursor+1] == '-';
                unread();
    ...
    private Node group0() {
        ...
        if (node == head) { root = tail;
            return node;    }
        if (head == tail) { root = node;
            return node;    }
        ...
        if (node instanceof Ques) {
            ...
            } else { head = new Branch(null, head, tail);
            }
            root = tail;
            return head;
        } else if (node instanceof Curly) {
            ...
            TreeInfo info = new TreeInfo();
            if (head.study(info)) { GroupTail temp = (GroupTail) tail;
                head = root = new GroupCurly(head.next, curly.cmin,
                                   ...
                                   ((GroupTail)tail).groupIndex,
                                             capturingGroup);
                return head;
            } else { int temp = ((GroupHead) head).localIndex;
                Loop loop;
    ...
    private CharProperty newSingle(final int ch) {
        ...
        if (isSupplementary(ch))
            return new SingleS(ch);    return new Single(ch);         }
    ...
    static final class Branch extends Node {
        ...
        boolean study(TreeInfo info) {
            ...
            int minL2 = Integer.MAX_VALUE; int maxL2 = -1;
            for (int n = 0; n < size; n++) {
                info.reset();
                if (atoms[n] != null)
                    atoms[n].study(info);
    ...
    static final class GroupHead extends Node {
        ...
        boolean matchRef(Matcher matcher, int i, CharSequence seq) {
            int save = matcher.locals[localIndex];
            matcher.locals[localIndex] = ~i; boolean ret = next.match(matcher, i, seq);
            matcher.locals[localIndex] = save;
            return ret;
        }
    }
    ...
    static final class GroupTail extends Node {
        ...
        boolean match(Matcher matcher, int i, CharSequence seq) {
            ...
            if (tmp >= 0) { int groupStart = matcher.groups[groupIndex];
    ...
    static class Loop extends Node {
        Node body;
        int countIndex; int beginIndex; int cmin, cmax;
    ...
    private static class CharPropertyNames {
        ...
        static {
            ...
            defCategory("C", ((1<<Character.CONTROL)     |
                              ...
                              (1<<Character.SURROGATE))); defCategory("P", ((1<<Character.DASH_PUNCTUATION)      |
            ...
            defRange("L1", 0x00, 0xFF); map.put("all", new CharPropertyFactory() {
                    CharProperty make() { return new All(); }});

            defRange("ASCII", 0x00, 0x7F);   defCtype("Alnum", ASCII.ALNUM);  defCtype("Alpha", ASCII.ALPHA);  defCtype("Blank", ASCII.BLANK);  defCtype("Cntrl", ASCII.CNTRL);  defRange("Digit", '0', '9');     defCtype("Graph", ASCII.GRAPH);  defRange("Lower", 'a', 'z');     defRange("Print", 0x20, 0x7E);   defCtype("Punct", ASCII.PUNCT);  defCtype("Space", ASCII.SPACE);  defRange("Upper", 'A', 'Z');     defCtype("XDigit",ASCII.XDIGIT); defClone("javaLowerCase", new CloneableProperty() {
                boolean isSatisfiedBy(int ch) {
            ...
            public boolean hasNext() {
                ...
                while (matcher.find()) {
                    ...
                    if (!nextElement.isEmpty()) {
                        return true;
                    } else if (current > 0) { emptyElementCount++;
                    }
                }
