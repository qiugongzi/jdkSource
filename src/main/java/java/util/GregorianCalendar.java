...
public static final int BC = 0;
    ...
    private static final int EPOCH_OFFSET   = 719163; private static final int EPOCH_YEAR     = 1970;

    static final int MONTH_LENGTH[]
        = {31,28,31,30,31,30,31,31,30,31,30,31}; static final int LEAP_MONTH_LENGTH[]
        = {31,29,31,30,31,30,31,31,30,31,30,31}; private static final int  ONE_SECOND = 1000;
    ...
    static final int LEAST_MAX_VALUES[] = {
        CE,             292269054,      DECEMBER,       52,             4,              28,             365,            SATURDAY,       4,              PM,             11,             23,             59,             59,             999,            14*ONE_HOUR,    20*ONE_MINUTE   };
    static final int MAX_VALUES[] = {
        CE,             292278994,      DECEMBER,       53,             6,              31,             366,            SATURDAY,       6,              PM,             11,             23,             59,             59,             999,            14*ONE_HOUR,    2*ONE_HOUR      };
...
private long gregorianCutover = DEFAULT_GREGORIAN_CUTOVER;
    ...
    private transient long gregorianCutoverDate =
        (((DEFAULT_GREGORIAN_CUTOVER + 1)/ONE_DAY) - 1) + EPOCH_OFFSET; private transient int gregorianCutoverYear = 1582;
...
public void setGregorianChange(Date date) {
    ...
    public boolean isLeapYear(int year) {
        ...
        if (gregorianCutoverYear == gregorianCutoverYearJulian) {
            BaseCalendar.Date d = getCalendarDate(gregorianCutoverDate); gregorian = d.getMonth() < BaseCalendar.MARCH;
        } else {
    ...
    public void add(int field, int amount) {
        ...
        if (field == YEAR) {
            ...
            case DAY_OF_MONTH: case DAY_OF_YEAR:
            case DAY_OF_WEEK:
                break;
            ...
            fd += delta; int zoneOffset = internalGet(ZONE_OFFSET) + internalGet(DST_OFFSET);
    ...
    public void roll(int field, int amount) {
        ...
        case HOUR_OF_DAY:
            {
                int unit = max + 1; int h = internalGet(field);
        ...
        case WEEK_OF_MONTH:
            {
                ...
                long fd = getCurrentFixedDate();
                long month1;     int monthLength; if (isCutoverYear) {
        ...
        case DAY_OF_WEEK:
            {
                if (!isCutoverYear(cdate.getNormalizedYear())) {
                    ...
                    if (weekOfYear > 1 && weekOfYear < 52) {
                        set(WEEK_OF_YEAR, weekOfYear); max = SATURDAY;
                        break;
                    }
                }
        ...
        case DAY_OF_WEEK_IN_MONTH:
            {
                min = 1; if (!isCutoverYear(cdate.getNormalizedYear())) {
        ...
        case WEEK_OF_MONTH:
            {
                if (!gc.isCutoverYear(normalizedYear)) {
                    ...
                    int nDaysFirstWeek = 7 - dayOfWeek; value = 3;
    ...
    public int getWeekYear() {
        int year = get(YEAR); if (internalGetEra() == BCE) {
            year = 1 - year;
        }
...
transient private long cachedFixedDate = Long.MIN_VALUE;
    ...
    private int computeFields(int fieldMask, int tzMask) {
        ...
        int month =  cdate.getMonth() - 1; int dayOfMonth = cdate.getDayOfMonth();
        ...
        if ((fieldMask & (HOUR_OF_DAY_MASK|AM_PM_MASK|HOUR_MASK
            ...
            if (timeOfDay != 0) {
                ...
                internalSet(AM_PM, hours / 12); internalSet(HOUR, hours % 12);
