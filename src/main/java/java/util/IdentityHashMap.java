    ...
    implements Map<K,V>, java.io.Serializable, Cloneable
{
    ...
    transient Object[] table; int size;
    ...
    private boolean resize(int newCapacity) {
        ...
        if (oldLength == 2 * MAXIMUM_CAPACITY) { if (size == MAXIMUM_CAPACITY - 1)
    ...
    public void putAll(Map<? extends K, ? extends V> m) {
        ...
        if (n == 0)
            return;
        if (n > size)
            resize(capacity(n)); for (Entry<? extends K, ? extends V> e : m.entrySet())
    ...
    private abstract class IdentityHashMapIterator<T> implements Iterator<T> {
        int index = (size != 0 ? 0 : table.length); int expectedModCount = modCount; int lastReturnedIndex = -1;      boolean indexValid; Object[] traversalTable = table; public boolean hasNext() {
    ...
    static class IdentityHashMapSpliterator<K,V> {
        ...
        int index;             int fence;             int est;               int expectedModCount;  IdentityHashMapSpliterator(IdentityHashMap<K,V> map, int origin,
