...
public class ConcurrentSkipListMap<K,V> extends AbstractMap<K,V>
    ...
    private Node<K,V> findPredecessor(Object key, Comparator<? super K> cmp) {
        if (key == null)
            throw new NullPointerException(); for (;;) {
    ...
    private Node<K,V> findNode(Object key) {
        ...
        outer: for (;;) {
            for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {
                ...
                Node<K,V> f = n.next;
                if (n != b.next)                break;
                if ((v = n.value) == null) {    n.helpDelete(b, f);
                    break;
                }
    ...
    private V doPut(K key, V value, boolean onlyIfAbsent) {
        ...
        if ((rnd & 0x80000001) == 0) { int level = 1, max;
            while (((rnd >>>= 1) & 1) != 0)
                ++level;
            Index<K,V> idx = null;
            ...
            else { level = max + 1; @SuppressWarnings("unchecked")Index<K,V>[] idxs =
        ...
        outer: for (;;) {
            for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {
                ...
                if (!n.appendMarker(f) || !b.casNext(n, f))
                    findNode(key);                  else {
                    findPredecessor(key, cmp);      if (head.right == null)
                        tryReduceLevel();
                }
                @SuppressWarnings("unchecked") V vv = (V)v;
                return vv;
    ...
    private void tryReduceLevel() {
        ...
        if (h.level > 3 &&
            ...
            d.right == null &&
            h.right == null &&
            casHead(h, d) && h.right != null) casHead(d, h);   }
    ...
    private Map.Entry<K,V> doRemoveLastEntry() {
        for (;;) {
            ...
            for (;;) {
                ...
                K key = n.key;
                if (!n.appendMarker(f) || !b.casNext(n, f))
                    findNode(key);                  else {                              findPredecessor(key, comparator);
                    if (head.right == null)
                        tryReduceLevel();
                }
                @SuppressWarnings("unchecked") V vv = (V)v;
    ...
    private static final int GT = 0; final Node<K,V> findNear(K key, int rel, Comparator<? super K> cmp) {
        if (key == null)
            throw new NullPointerException();
        for (;;) {
            for (Node<K,V> b = findPredecessor(key, cmp), n = b.next;;) {
    ...
    public void clear() {
        for (;;) {
            ...
            if (d != null)
                casHead(h, d);            else if ((b = h.node) != null && (n = b.next) != null) {
                Node<K,V> f = n.next;     if (n == b.next) {
                    Object v = n.value;
                    if (v == null)
                        n.helpDelete(b, f);
    ...
    abstract static class CSLMSpliterator<K,V> {
        final Comparator<? super K> comparator;
        final K fence;     Index<K,V> row;    Node<K,V> current; int est;           CSLMSpliterator(Comparator<? super K> comparator, Index<K,V> row,
    ...
    final KeySpliterator<K,V> keySpliterator() {
        Comparator<? super K> cmp = comparator;
        for (;;) { HeadIndex<K,V> h; Node<K,V> p;
