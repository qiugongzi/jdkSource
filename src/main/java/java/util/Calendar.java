...
public abstract class Calendar implements Serializable, Cloneable, Comparable<Calendar> {
    ...
    public static class Builder {
        private static final int NFIELDS = FIELD_COUNT + 1; private static final int WEEK_YEAR = FIELD_COUNT;
        ...
        public Builder setCalendarType(String type) {
            if (type.equals("gregorian")) { type = "gregory";
            }
    ...
    public final void clear()
    {
        for (int i = 0; i < fields.length; ) {
            stamp[i] = fields[i] = 0; isSet[i++] = false;
        }
    ...
    boolean checkDisplayNameParams(int field, int style, int minStyle, int maxStyle,
        ...
        int baseStyle = getBaseStyle(style); if (field < 0 || field >= fields.length ||
    ...
    private String[] getFieldStrings(int field, int style, DateFormatSymbols symbols) {
        int baseStyle = getBaseStyle(style); if (baseStyle == NARROW_FORMAT) {
            return null;
        }
    ...
    protected void complete()
    {
        ...
        if (!areFieldsSet || !areAllFieldsSet) {
            computeFields(); areAllFieldsSet = areFieldsSet = true;
        }
    }
