...
public class Collections {
    ...
    public static void shuffle(List<?> list) {
        Random rnd = r;
        if (rnd == null)
            r = rnd = new Random(); shuffle(list, rnd);
    }
            ...
            for (int candidate = 0; candidate <= maxCandidate; candidate++) {
                for (int i=0, j=candidate; i<targetSize; i++, j++)
                    if (!eq(target.get(i), source.get(j)))
                        continue nextCand;  return candidate;  }
        } else {  ListIterator<?> si = source.listIterator();
        nextCand:
            for (int candidate = 0; candidate <= maxCandidate; candidate++) {
    ...
    public static int lastIndexOfSubList(List<?> source, List<?> target) {
        ...
        if (sourceSize < INDEXOFSUBLIST_THRESHOLD ||
            source instanceof RandomAccess) {   nextCand:
    ...
    static class SynchronizedCollection<E> implements Collection<E>, Serializable {
        ...
        final Collection<E> c;  final Object mutex;     SynchronizedCollection(Collection<E> c) {
    ...
    private static class SynchronizedMap<K,V>
        ...
        private final Map<K,V> m;     final Object      mutex;        SynchronizedMap(Map<K,V> m) {
    ...
    private static class SetFromMap<E> extends AbstractSet<E>
        implements Set<E>, Serializable
    {
        private final Map<E, Boolean> m;  private transient Set<E> s;       SetFromMap(Map<E, Boolean> map) {
            if (!map.isEmpty())
